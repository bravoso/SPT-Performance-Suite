...
internal static class Program
{
    ...
    private static void TestPercentiles()
    {
        Near(25, Percentiles.FromSorted(new[] { 10d, 20d, 30d, 40d }, 4, 0.5));
    }
    ...
    private static void TestSerialization()
    {
        var export = new BenchmarkExport
        {
            ...
            Samples = new[]
            {
                new BenchmarkSample
                {
                    ...
                    MainThreadMs = 7,
                    GpuFrameMs = 4,
                    WaitForTargetFpsMs = 2,
                    PlayerCount = 2,
                    AiCount = 1,
                    BakedCullingEntityCount = 1,
                    BakedHiddenEntityCount = 1,
                    ShadowEffectiveDistance = 75,
                    ShadowDisabledRendererCount = 20,
                    SkinningModifiedRendererCount = 4,
                    RemoteLodFarAiCount = 3,
                    RemoteLodForcedGroupCount = 8,
                    DeclutterHiddenRendererCount = 120,
                    AreaLightReusedCommandBuffers = 200,
                    AreaLightRebuiltCommandBuffers = 50,
                    RemoteBudgetedCharacterCount = 6,
                    RemoteSkippedPropUpdates = 42,
                    RemoteSkippedPresentationUpdates = 84,
                    OptimizationsEnabled = true,
                    PipScopeActive = true,
                    PipRenderingDisabled = true,
                    PipScopeSourceResolution = 1024,
                    PipScopeOptimizedResolution = 512,
                    PipScopeRenderedFrames = 20,
                    PipScopeReusedFrames = 40,
                    PipScopeAverageRenderMs = 1.25,
                    CompatibilityFastWorldLookups = 8,
                },
                new BenchmarkSample
                {
        ...
        BenchmarkSerializer.WriteCsv(csv, export);
        var json = new StringWriter();
        BenchmarkSerializer.WriteJson(json, export);
    ...
    private static void TestEntityClassification()
    {
        ...
        Equal(EntityKind.Corpse, EntityClassifierLogic.Classify(new EntitySignals(true, false, false, false, null, true)));
        Equal(EntityKind.NonPlayer, EntityClassifierLogic.Classify(new EntitySignals(false, false, false, false, null, false)));
        Equal(
            EntityKind.UnknownPlayerLikeEntity,
            EntityClassifierLogic.Classify(new EntitySignals(true, false, false, false, null, false))
        );
    }
    ...
    private static void TestConfiguration()
    {
        ...
        Near(5, value.CaptureSeconds);
        Near(1000, value.ShadowDistance);
        Near(0.1, value.UpdateIntervalSeconds);
        Near(1000, value.ShadowMinimumDistance);
        Near(240, value.ShadowTargetFps);
        Near(20, value.SkinningDistance);
        Near(0.05, value.SkinningUpdateIntervalSeconds);
        Near(10, value.SkinningOffscreenHoldSeconds);
    }
    ...
    private static void TestAdaptiveDistance()
    {
        ...
        for (int i = 0; i < 120; i++)
        {
            controller.Update(1.0 / 60.0, 12, 120, 60, 60);
        }

        Near(120, controller.EffectiveDistance);
        for (int i = 0; i < 240; i++)
        {
            controller.Update(1.0 / 60.0, 25, 120, 60, 60);
        }
        ...
        {
            controller.Update(1.0 / 120.0, 8, 120, 60, 60);
        }
    ...
    private static void TestScheduler()
    {
        ...
        True(scheduler.IsDue(0));
        True(!scheduler.IsDue(0.1));
        True(scheduler.IsDue(0.25));
    }
    ...
    private static void TestFrameWorkBudget()
    {
        ...
        {
            if (FrameWorkBudget.ShouldRun(frame, 7, 4))
            {
                runs++;
            }
        }
    ...
    private static void TestStateCache()
    {
        ...
        int restored = 0;
        True(cache.Remember("renderer", 2));
        True(!cache.Remember("renderer", 3));
