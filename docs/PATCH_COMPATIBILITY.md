...
The suite installs only prefix/postfix timing patches and only when `Diagnostics.MethodTimingEnabled=true`. Each target is resolved with an exact parameter and return signature. Missing or mismatched targets are skipped. Prefix/postfix errors fail open and a three-error circuit breaker disables timing while originals continue. Version 0.1.1 also instruments verified high-level `GameWorld` and `Player` update/presentation methods listed in `CANDIDATE_HOT_PATHS.md`.
