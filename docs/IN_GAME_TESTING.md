...
6. Enter a raid. Confirm one raid-start log, gradual entity counts, F8 overlay toggle, and unavailable counters displaying `n/a` without errors.
7. Press F9, wait for completion, and verify matching CSV/JSON files plus map/feature state.
8. Press F10 and verify the diagnostic report, runtime profiler list, four exact timing targets when timing is enabled, and their live Harmony owners.
9. Enable `RemoteCharacterShadowsDryRun=true` and the experiment. Press F12 as needed. Verify counters change but renderer shadows do not.
