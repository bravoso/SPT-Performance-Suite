...
5. Confirm `OldCpuAggressive` and `AllOptimizationsEnabled = true` are selected. The overlay must say `OPTIMIZATIONS ON`.
...
11. Test a Vudu or TAC30 with PiP-Disabler disabled. The magnified lens must remain a true independently rendered view. Compare Num4 ON/OFF while scoped: ON should report the source and optimized resolution. HDR and scope motion must remain identical to vanilla; thermal/NVG scopes must report bypass and remain unchanged.
