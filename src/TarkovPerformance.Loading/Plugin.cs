...
/// <summary>
/// Coordinates client/headless loading settings, stage timing, and guarded loading-path experiments.
/// </summary>
/// <remarks>
/// Unity and EFT objects remain on the main thread unless a replacement explicitly operates on independent,
/// read-only data. Every experimental prefix fails open to EFT's original implementation when its assumptions
/// are not satisfied.
/// </remarks>
public sealed class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        ApplyLoadingBoost();
        if (_optimizeSyncRequests.Value)
        {
            PatchSynchronousRequests();
        }

        TryPatchLoadingOptimizations();
        if (_writeReports.Value)
        {
    ...
    private void Update()
    {
        ...
        LoadingPhase detected =
            world == null ? LoadingPhase.StartupOrMenu
            : HasLocalPlayer(world) ? LoadingPhase.InRaid
            : LoadingPhase.RaidLoading;
        ...
        if (detected != _phase)
        {
            ...
            Logger.LogInfo(
                "Loading phase: "
                    ...
                    + detected
                    + " at "
                    + _lifetime.Elapsed.TotalSeconds.ToString("F2", CultureInfo.InvariantCulture)
                    + " s"
            );
            ...
            {
                ApplyLoadingBoost();
                if (previous == LoadingPhase.InRaid)
                {
                    _postRaidReportAt = now + 8f;
                }
            }
        }
    ...
    private void BindConfiguration()
    {
        ...
        _enabled = Config.Bind(
            ...
            "Enabled",
            true,
            "Boost game/profile/raid loading. Gameplay settings are restored when your player is ready."
        );
        _optimizeSyncRequests = Config.Bind(
            section,
            "Remove redundant request worker hop",
            true,
            "Uses reusable background request workers so Unity cannot deadlock while waiting for synchronous SPT requests."
        );
        _writeReports = Config.Bind(
            section,
            "Write loading reports",
            false,
            "Writes request and phase timings under TarkovPerformanceSuite/loading-reports. Disabled by default for normal play."
        );
        _raiseProcessPriority = Config.Bind(
            section,
            "Above-normal process priority while loading",
            true,
            "Gives loading work preference, then restores the previous process priority in raid."
        );
        _raiseMainThreadPriority = Config.Bind(
            section,
            "Above-normal main thread while loading",
            true,
            "Prioritizes Unity integration work during loading only."
        );
        _uploadTimeSlice = Config.Bind(
            section,
            "Asset upload time per frame ms",
            16,
            new ConfigDescription(
                "Unity main-thread time allowed for asset uploads while loading. EFT doubles this to 32 ms during raid scene loading, which is Unity's supported maximum.",
                new AcceptableValueRange<int>(2, 16)
            )
        );
        _uploadBufferMb = Config.Bind(
            section,
            "Asset upload buffer MB",
            256,
            new ConfigDescription(
                "System-memory staging buffer used while loading textures and meshes. EFT temporarily doubles this during raid scene loading.",
                new AcceptableValueRange<int>(32, 256)
            )
        );
        _workerThreadsPerCore = Config.Bind(
            section,
            "Minimum workers per logical CPU",
            2,
            new ConfigDescription(
                "Prevents the worker pool from slowly ramping up during loading bursts.",
                new AcceptableValueRange<int>(1, 4)
            )
        );
        _useAllUnityJobWorkers = Config.Bind(
            section,
            "Use all Unity job workers while loading",
            true,
            "Lets Unity use its hardware-specific maximum worker count for loading jobs, then restores the previous count in raid."
        );
        _concurrentSceneBundlePreload = Config.Bind(
            section,
            "Preload raid scene bundles concurrently",
            true,
            "Loads scene bundle files in small concurrent batches, then lets EFT integrate and activate every scene in its original order."
        );
        _sceneBundlePreloadConcurrency = Config.Bind(
            section,
            "Scene bundle preload concurrency",
            4,
            new ConfigDescription(
                "Maximum scene bundle files prepared at once. Scene creation and activation remain single-file and ordered.",
                new AcceptableValueRange<int>(2, 8)
            )
        );
        _parallelResourceClassification = Config.Bind(
            section,
            "Parallel item and bundle preparation",
            true,
            "Uses several CPU threads for the read-only item-to-bundle classification pass before pools are created."
        );
        _parallelLootSerialization = Config.Bind(
            section,
            "Parallel headless loot preparation",
            true,
            "Splits Fika host/headless read-only loot serialization across CPU threads before it is sent to clients."
        );
        _linearStaticLootMatching = Config.Bind(
            section,
            "Fast static loot and container matching",
            true,
            "Replaces EFT's containers-times-loot nested scan with a single ID lookup pass while loading the map."
        );
        _resourceClassificationWorkers = Config.Bind(
            section,
            "Item preparation worker limit",
            0,
            new ConfigDescription(
                "0 uses all logical CPUs. Lower this if loading competes with another SPT instance on the same PC.",
                new AcceptableValueRange<int>(0, 64)
            )
        );
        _resourceClassificationThreshold = Config.Bind(
            section,
            "Parallel item preparation minimum count",
            128,
            new ConfigDescription(
                "Small lists stay single-threaded to avoid parallel scheduling overhead.",
                new AcceptableValueRange<int>(32, 2048)
            )
        );
        _slowRequestMs = Config.Bind(
            section,
            "Slow request warning ms",
            250,
            new ConfigDescription("Requests slower than this are highlighted in the log/report.", new AcceptableValueRange<int>(50, 5000))
        );
    }
    ...
    private void ApplyLoadingBoost()
    {
        ...
        try
        {
            ...
            int desiredWorkers = Math.Min(
                maxWorkers,
                Math.Max(_original.MinWorkerThreads, Environment.ProcessorCount * _workerThreadsPerCore.Value)
            );
            ...
            if (_raiseMainThreadPriority.Value)
            {
                Thread.CurrentThread.Priority = SystemThreadPriority.AboveNormal;
            }
            ...
            if (_raiseProcessPriority.Value)
            {
                ...
                {
                    Logger.LogDebug("Could not change process priority: " + ex.Message);
                }
            }
            _boostApplied = true;
            Logger.LogInfo(
                "Loading boost applied: upload "
    ...
    {
        if (!_boostApplied || !_haveOriginal)
        {
            return;
        }
    ...
    {
        if (_optimizationPatchesInstalled)
        {
            return;
        }
        ...
        try
        {
            ...
            _harmony.Patch(
                ...
                prefix: new HarmonyMethod(
                    ...
                    nameof(LoadingOptimizationPatches.ConcurrentSceneBundlePreloadPrefix)
                )
            );
            _harmony.Patch(
                classifyResources,
                prefix: new HarmonyMethod(
                    typeof(LoadingOptimizationPatches),
                    nameof(LoadingOptimizationPatches.ParallelResourceClassificationPrefix)
                )
            );
            _harmony.Patch(
                serializeLoot,
                prefix: new HarmonyMethod(
                    typeof(LoadingOptimizationPatches),
                    nameof(LoadingOptimizationPatches.ParallelLootSerializationPrefix)
                )
            );
            _harmony.Patch(
                matchStaticLoot,
                prefix: new HarmonyMethod(
                    typeof(LoadingOptimizationPatches),
                    nameof(LoadingOptimizationPatches.FastStaticLootMatchingPrefix)
                )
            );
            _optimizationPatchesInstalled = true;
            Logger.LogInfo(
                "Loading parallelism attached (concurrent scene-bundle preload + read-only item/bundle and headless loot preparation). Scene integration remains ordered."
            );
        }
        catch (Exception ex)
        {
    ...
    private void TryPatchLoadingStages()
    {
        ...
        try
        {
            Assembly fika = AppDomain
                .CurrentDomain.GetAssemblies()
                .FirstOrDefault(assembly => string.Equals(assembly.GetName().Name, "Fika.Core", StringComparison.OrdinalIgnoreCase));
            ...
            if (lootProgress == null || mapLoading == null || mapCaching == null)
            {
                return;
            }
            ...
            _harmony.Patch(
                lootProgress,
                postfix: new HarmonyMethod(typeof(LoadingStagePatches), nameof(LoadingStagePatches.LootProgressPostfix))
            );
            _harmony.Patch(
                mapLoading,
                postfix: new HarmonyMethod(typeof(LoadingStagePatches), nameof(LoadingStagePatches.MapLoadingPostfix))
            );
            _harmony.Patch(
                mapCaching,
                postfix: new HarmonyMethod(typeof(LoadingStagePatches), nameof(LoadingStagePatches.MapCachingPostfix))
            );
    ...
    internal static bool ShouldPreloadSceneBundles(int sceneCount)
    {
        return Instance != null
            && Instance._boostApplied
            && Instance._concurrentSceneBundlePreload != null
            && Instance._concurrentSceneBundlePreload.Value
            && sceneCount > 1;
    }

    internal static int SceneBundlePreloadConcurrency
    {
        get { return Instance == null ? 2 : Math.Max(2, Math.Min(8, Instance._sceneBundlePreloadConcurrency.Value)); }
    }
    ...
    internal static bool ShouldParallelizeResourceClassification(int itemCount)
    {
        return Instance != null
            && Instance._boostApplied
            && Instance._parallelResourceClassification != null
            && Instance._parallelResourceClassification.Value
            && itemCount >= Instance._resourceClassificationThreshold.Value;
    }
    ...
    internal static bool ShouldParallelizeLootSerialization(int itemCount)
    {
        return Instance != null
            && Instance._boostApplied
            && Instance._parallelLootSerialization != null
            && Instance._parallelLootSerialization.Value
            && itemCount >= Instance._resourceClassificationThreshold.Value;
    }
    ...
    internal static bool ShouldUseFastStaticLootMatching
    {
        get
        {
            return Instance != null
                && Instance._boostApplied
                && Instance._linearStaticLootMatching != null
    ...
    private void RecordStage(string stage, float progress)
    {
        ...
        lock (_stageGate)
        {
            ...
            if (current == null || !string.Equals(current.Name, stage, StringComparison.Ordinal))
            {
                ...
                Logger.LogInfo(
                    "Detailed loading stage: " + stage + " started at " + now.ToString("F2", CultureInfo.InvariantCulture) + " s"
                );
            }
    ...
    private void FinishLoadingStage()
    {
        lock (_stageGate)
        {
            if (_loadingStages.Count > 0)
            {
                _loadingStages[_loadingStages.Count - 1].EndSeconds = _lifetime.Elapsed.TotalSeconds;
            }
        }
    }
    ...
    {
        MethodInfo target = typeof(RequestHandler)
    ...
    {
        Plugin plugin = Instance;
        if (plugin == null || started == 0L || plugin._writeReports == null || !plugin._writeReports.Value)
        {
            return;
        }
    ...
    private void RecordRequest(RequestSample sample)
    {
        ...
        while (count > 4096 && _requests.TryDequeue(out _))
        {
    ...
    {
        if (_writeReports == null || !_writeReports.Value)
        {
            return;
        }
        ...
        try
        {
            ...
            text.AppendLine(
                "Process uptime observed: " + _lifetime.Elapsed.TotalSeconds.ToString("F2", CultureInfo.InvariantCulture) + " s"
            );
            text.AppendLine(
            ...
            text.AppendLine(
                "Managed memory: " + (GC.GetTotalMemory(false) / 1048576d).ToString("F1", CultureInfo.InvariantCulture) + " MB"
            );
            text.AppendLine("Logical CPUs: " + Environment.ProcessorCount);
            text.AppendLine(
                "Loading upload budget: "
                    + _uploadTimeSlice.Value
                    + " ms / "
                    + _uploadBufferMb.Value
                    + " MB (EFT doubles these during raid scene loading, capped by the 32 ms / 512 MB preset)"
            );
            text.AppendLine("Unity job workers requested: " + (_useAllUnityJobWorkers.Value ? "maximum" : "unchanged"));
            text.AppendLine(
                "Concurrent scene bundle preload: "
                    + Volatile.Read(ref _sceneBundlePreloadPasses)
                    + " passes, "
                    + Volatile.Read(ref _sceneBundlesPreloaded)
                    + " bundles, "
                    + (Volatile.Read(ref _sceneBundlePreloadTicks) * 1000.0 / Stopwatch.Frequency).ToString(
                        "F2",
            ...
            text.AppendLine(
                "Parallel item preparation: "
                    + Volatile.Read(ref _parallelClassificationPasses)
                    + " passes, "
                    + Volatile.Read(ref _parallelClassificationItems)
                    + " items, "
                    + (Volatile.Read(ref _parallelClassificationTicks) * 1000.0 / Stopwatch.Frequency).ToString(
                        "F2",
                        CultureInfo.InvariantCulture
                    )
                    + " ms wall time"
            ...
            text.AppendLine(
                "Parallel headless loot preparation: "
                    + Volatile.Read(ref _parallelLootPasses)
                    + " passes, "
                    + Volatile.Read(ref _parallelLootItems)
                    + " top-level loot entries, "
                    + (Volatile.Read(ref _parallelLootSerializationTicks) * 1000.0 / Stopwatch.Frequency).ToString(
                        "F2",
                        CultureInfo.InvariantCulture
                    )
                    + " ms wall time"
            ...
            text.AppendLine(
                "Fast static loot matching: "
                    + Volatile.Read(ref _staticLootMatchingPasses)
                    + " passes, "
                    + Volatile.Read(ref _staticLootMatchingEntries)
                    + " total lookup entries, "
                    + (Volatile.Read(ref _staticLootMatchingTicks) * 1000.0 / Stopwatch.Frequency).ToString(
                        "F2",
                        CultureInfo.InvariantCulture
                    )
                    + " ms wall time"
            ...
            lock (_stageGate)
            {
                if (_loadingStages.Count == 0)
                {
                    text.AppendLine("  unavailable (Fika stage hooks were not active)");
                }
                ...
                for (int i = 0; i < _loadingStages.Count; i++)
                {
                    ...
                    text.AppendLine(
                        (stage.EndSeconds - stage.StartSeconds).ToString("F2", CultureInfo.InvariantCulture).PadLeft(8)
                            + " s | "
                            + stage.Name
                            + " | final progress "
                            + (stage.LastProgress * 100f).ToString("F1", CultureInfo.InvariantCulture)
                            + "%"
                    );
                }
            }
            ...
            {
                text.AppendLine(
                    sample.ElapsedMs.ToString("F1", CultureInfo.InvariantCulture).PadLeft(9)
                        + " ms | "
                        + sample.Phase
                        + " | "
                        + sample.Method
            ...
            string file = Path.Combine(
                ...
                "loading-"
                    ...
                    + "-"
                    + (IsHeadlessProcess() ? "headless" : "client")
                    + "-"
                    + reason
                    + ".txt"
            );
    ...
    /// <summary>Identifies broad loading phases for process-priority and report decisions.</summary>
    ...
    /// <summary>Stores one synchronous SPT HTTP-wrapper measurement for the loading report.</summary>
    ...
    /// <summary>Tracks the observed start, end, and latest progress of one EFT loading stage.</summary>
    ...
    /// <summary>Captures all process and Unity loading settings changed by this plugin for restoration.</summary>
    private readonly struct LoadingSnapshot
    {
        private LoadingSnapshot(
            ...
            int uploadBuffer,
            bool persistent,
            UnityThreadPriority loadingPriority,
            int minWorkers,
            int minIo,
            int unityJobWorkers,
            ProcessPriorityClass processPriority,
            SystemThreadPriority mainThreadPriority
        )
        {
        ...
        internal static LoadingSnapshot Capture()
        {
            ...
            try
            {
                priority = Process.GetCurrentProcess().PriorityClass;
            }
            catch
            {
        ...
        internal void Apply()
        {
            ...
            try
            {
                Thread.CurrentThread.Priority = MainThreadPriority;
            }
            catch { }
        }
    }
...
/// <summary>Forwards EFT loading progress callbacks to the stage recorder without changing their results.</summary>
public static class LoadingStagePatches
{
    public static void MapLoadingPostfix(float pr)
    {
        Plugin.RecordLoadingProgress("Map loading / preparing", pr);
    }

    public static void MapCachingPostfix(float totalProgress)
    {
        Plugin.RecordLoadingProgress("Map culling cache", totalProgress);
    }

    public static void LootProgressPostfix(LoadingProgressStruct p)
    {
        Plugin.RecordLoadingProgress(p.Stage.ToString(), p.Progress);
    }
}

/// <summary>
/// Contains opt-in loading replacements whose thread-safety assumptions are checked before suppressing EFT code.
/// </summary>
public static class LoadingOptimizationPatches
{
    public static bool ConcurrentSceneBundlePreloadPrefix(
        GClass2287 __instance,
        IList<ResourceKey> scenes,
        IProgress<float> progress,
        ref Task __result
    )
    {
        if (scenes == null || !Plugin.ShouldPreloadSceneBundles(scenes.Count))
        {
            return true;
        }
    ...
    private static async Task LoadScenesWithConcurrentBundlePreload(GClass2287 loader, IList<ResourceKey> scenes, IProgress<float> progress)
    {
        ...
        if (loader.CancellationToken_0.IsCancellationRequested)
        {
            return;
        }
        ...
        for (int index = 1; index < scenes.Count; index++)
        {
            ...
            GClass3968 scene = LoadSceneClass.LoadScene(
                loader.IAssetsManager,
                scenes[index],
    ...
    private static HashSet<string> GetBundleFootprint(AssetsManagerClass assetsManager, string bundleName)
    {
        ...
        try
        {
            result.UnionWith(assetsManager.BundlesManager.FindDependences(bundleName));
        }
        catch { }
        return result;
    }

    public static bool ParallelResourceClassificationPrefix(
        PoolManagerClass.Class1448 __instance,
        ICollection<ResourceKey> resources,
        ref List<PoolManagerClass.GStruct281> __result
    )
    {
        if (resources == null || !Plugin.ShouldParallelizeResourceClassification(resources.Count))
        {
            return true;
        }

        ResourceKey[] unique = resources.Distinct().ToArray();
        if (!Plugin.ShouldParallelizeResourceClassification(unique.Length))
        {
            return true;
        }
        ...
        try
        {
            ...
            Parallel.For(
                0,
                unique.Length,
                new ParallelOptions { MaxDegreeOfParallelism = Plugin.ResourceClassificationWorkers },
    ...
    public static bool ParallelLootSerializationPrefix(
        IEnumerable<LootItemPositionClass> lootData,
        ISearchController searchController,
        ref GClass1947 __result
    )
    {
        if (lootData == null)
        {
        ...
        if (!Plugin.ShouldParallelizeLootSerialization(source.Length))
        {
            return true;
        }
    ...
    {
        if (!Plugin.ShouldUseFastStaticLootMatching || lootItems == null || staticLootSpawns == null)
        {
            return true;
        }
        ...
        try
        {
            ...
            {
                if (lootItem != null && lootItem.Id != null)
                {
                    presentIds.Add(lootItem.Id);
                }
            }
            ...
            for (int index = 0; index < staticLootSpawns.Length; index++)
            {
                ...
                if (staticLoot == null || presentIds.Contains(staticLoot.Id))
                {
                    continue;
                }
                ...
                UnityEngine.Debug.LogWarning(
                    "Static loot " + staticLoot.Id + " not found in raid data; disabling its linked scene objects.",
                    gameObject
                );
...
/// <summary>Moves synchronous SPT request wrappers onto dedicated workers while preserving blocking semantics.</summary>
public static class RequestPatches
{
    public static bool GetDataPrefix(string path, ref byte[] __result)
    {
        ...
        try
        {
            __result = DedicatedRequestWorker.Run(() => RequestHandler.GetDataAsync(path).ConfigureAwait(false).GetAwaiter().GetResult());
        }
        catch (Exception ex)
        {
            error = ex;
        ...
        {
            Plugin.EndRequest("GET", path, started, error);
        }
        return false;
    }
    ...
    public static bool GetJsonPrefix(string path, ref string __result)
    {
        ...
        try
        {
            __result = DedicatedRequestWorker.Run(() => RequestHandler.GetJsonAsync(path).ConfigureAwait(false).GetAwaiter().GetResult());
        }
        catch (Exception ex)
        {
            error = ex;
    ...
    public static bool PostJsonPrefix(string path, string json, ref string __result)
    {
        ...
        try
        {
            __result = DedicatedRequestWorker.Run(() =>
                RequestHandler.PostJsonAsync(path, json).ConfigureAwait(false).GetAwaiter().GetResult()
            );
        }
        catch (Exception ex)
        {
        ...
        {
            Plugin.EndRequest("POST", path, started, error);
        }
        return false;
    }
    ...
    public static bool PutJsonPrefix(string path, string json, ref string __result)
    {
        ...
        {
            __result = DedicatedRequestWorker.Run(() =>
                RequestHandler.PutJsonAsync(path, json).ConfigureAwait(false).GetAwaiter().GetResult()
            );
        }
        catch (Exception ex)
        {
        ...
        {
            Plugin.EndRequest("PUT", path, started, error);
        }
        return false;
    }
}

/// <summary>Owns bounded-lifetime HTTP workers used only by the synchronous request compatibility patches.</summary>
internal static class DedicatedRequestWorker
{
    ...
    private static void WorkerLoop()
    {
        ...
        try
        {
            foreach (IRequestWorkItem item in queue.GetConsumingEnumerable())
            {
                item.Execute();
            }
        }
    ...
    /// <summary>Provides a non-generic unit consumed by the shared worker queue.</summary>
    private interface IRequestWorkItem
    {
        void Execute();
    }

    /// <summary>Captures a typed request result or exception and returns it to the waiting EFT caller.</summary>
