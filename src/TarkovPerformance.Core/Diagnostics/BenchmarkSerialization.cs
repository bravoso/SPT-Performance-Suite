    ...
    public static class BenchmarkSerializer
    {
        ...
        public static void WriteCsv(TextWriter writer, BenchmarkExport export)
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                WriteNullableLong(writer, s.GcUsedMemory); writer.Write(',');
                WriteNullableLong(writer, s.GcReservedMemory); writer.Write(',');
                WriteNullableLong(writer, s.AppResidentMemory); writer.Write(',');
                WriteNullableLong(writer, s.DrawCalls); writer.Write(',');
                WriteNullableLong(writer, s.SetPassCalls); writer.Write(',');
                WriteInt(writer, s.PlayerCount); writer.Write(',');
                WriteInt(writer, s.AiCount); writer.Write(',');
                WriteInt(writer, s.VisibleAiCount); writer.Write(',');
                WriteInt(writer, s.BakedCullingEntityCount); writer.Write(',');
                WriteInt(writer, s.BakedHiddenEntityCount); writer.Write(',');
                WriteInt(writer, s.CorpseCount); writer.Write(',');
                WriteInt(writer, s.AnimatorCount); writer.Write(',');
                WriteInt(writer, s.SkinnedRendererCount); writer.Write(',');
                WriteInt(writer, s.ShadowRendererCount); writer.Write(',');
                WriteDouble(writer, s.ShadowEffectiveDistance); writer.Write(',');
                WriteInt(writer, s.ShadowDisabledRendererCount); writer.Write(',');
                WriteInt(writer, s.SkinningModifiedRendererCount); writer.Write(',');
                WriteInt(writer, s.RemoteLodMidAiCount); writer.Write(',');
                WriteInt(writer, s.RemoteLodFarAiCount); writer.Write(',');
                WriteInt(writer, s.RemoteLodForcedGroupCount); writer.Write(',');
                WriteInt(writer, s.RemoteLodModifiedRendererCount); writer.Write(',');
                WriteInt(writer, s.DeclutterHiddenRendererCount); writer.Write(',');
                WriteLong(writer, s.AreaLightReusedCommandBuffers); writer.Write(',');
                WriteLong(writer, s.AreaLightRebuiltCommandBuffers); writer.Write(',');
                WriteInt(writer, s.RemoteBudgetedCharacterCount); writer.Write(',');
                WriteInt(writer, s.RemoteCulledAnimatorCount); writer.Write(',');
                WriteLong(writer, s.RemoteSkippedPropUpdates); writer.Write(',');
                WriteLong(writer, s.RemoteSkippedTriggerSearches); writer.Write(',');
                WriteLong(writer, s.RemoteSkippedPresentationUpdates); writer.Write(',');
                WriteLong(writer, s.RemoteCombatShots); writer.Write(',');
                WriteLong(writer, s.RemoteSoundOnlyShots); writer.Write(',');
                WriteLong(writer, s.RemoteCombatSafetyBypasses); writer.Write(',');
                WriteLong(writer, s.RemoteCulledMuzzleEffects); writer.Write(',');
                WriteLong(writer, s.RemoteCulledImpactEffects); writer.Write(',');
                WriteLong(writer, s.RemoteCulledCasings); writer.Write(',');
                WriteInt(writer, s.RemoteCulledLights); writer.Write(',');
                writer.Write(s.RemoteSoundOnlyAuthority ? "true" : "false"); writer.Write(',');
                WriteDouble(writer, s.RemoteCombatDecisionMs); writer.Write(',');
                writer.Write(s.OptimizationsEnabled ? "true" : "false"); writer.Write(',');
                writer.Write(s.PipScopeActive ? "true" : "false"); writer.Write(',');
                writer.Write(s.PipRenderingDisabled ? "true" : "false"); writer.Write(',');
                WriteInt(writer, s.PipScopeSourceResolution); writer.Write(',');
                WriteInt(writer, s.PipScopeOptimizedResolution); writer.Write(',');
                WriteLong(writer, s.PipScopeRenderedFrames); writer.Write(',');
                WriteLong(writer, s.PipScopeReusedFrames); writer.Write(',');
                WriteDouble(writer, s.PipScopeAverageRenderMs); writer.Write(',');
                WriteLong(writer, s.CompatibilityFastWorldLookups); writer.Write(',');
                if (s.FikaServerFps.HasValue) WriteInt(writer, s.FikaServerFps.Value);
                writer.Write(',');
                ...
                if (i == 0) WriteCsvField(writer, export.EnabledFeatures ?? string.Empty);
                writer.WriteLine();
            }
        }
        ...
        public static void WriteJson(TextWriter writer, BenchmarkExport export)
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                writer.Write(",\"gcUsedMemory\":"); WriteJsonNullable(writer, s.GcUsedMemory);
                writer.Write(",\"gcReservedMemory\":"); WriteJsonNullable(writer, s.GcReservedMemory);
                writer.Write(",\"appResidentMemory\":"); WriteJsonNullable(writer, s.AppResidentMemory);
                ...
                writer.Write(",\"playerCount\":"); WriteInt(writer, s.PlayerCount);
                writer.Write(",\"aiCount\":"); WriteInt(writer, s.AiCount);
                writer.Write(",\"visibleAiCount\":"); WriteInt(writer, s.VisibleAiCount);
                writer.Write(",\"bakedCullingEntityCount\":"); WriteInt(writer, s.BakedCullingEntityCount);
                writer.Write(",\"bakedHiddenEntityCount\":"); WriteInt(writer, s.BakedHiddenEntityCount);
                writer.Write(",\"corpseCount\":"); WriteInt(writer, s.CorpseCount);
                writer.Write(",\"animatorCount\":"); WriteInt(writer, s.AnimatorCount);
                writer.Write(",\"skinnedRendererCount\":"); WriteInt(writer, s.SkinnedRendererCount);
                writer.Write(",\"shadowRendererCount\":"); WriteInt(writer, s.ShadowRendererCount);
                writer.Write(",\"shadowEffectiveDistance\":"); WriteDouble(writer, s.ShadowEffectiveDistance);
                writer.Write(",\"shadowDisabledRendererCount\":"); WriteInt(writer, s.ShadowDisabledRendererCount);
                writer.Write(",\"skinningModifiedRendererCount\":"); WriteInt(writer, s.SkinningModifiedRendererCount);
                writer.Write(",\"remoteLodMidAiCount\":"); WriteInt(writer, s.RemoteLodMidAiCount);
                writer.Write(",\"remoteLodFarAiCount\":"); WriteInt(writer, s.RemoteLodFarAiCount);
                writer.Write(",\"remoteLodForcedGroupCount\":"); WriteInt(writer, s.RemoteLodForcedGroupCount);
                writer.Write(",\"remoteLodModifiedRendererCount\":"); WriteInt(writer, s.RemoteLodModifiedRendererCount);
                writer.Write(",\"declutterHiddenRendererCount\":"); WriteInt(writer, s.DeclutterHiddenRendererCount);
                writer.Write(",\"areaLightReusedCommandBuffers\":"); WriteLong(writer, s.AreaLightReusedCommandBuffers);
                writer.Write(",\"areaLightRebuiltCommandBuffers\":"); WriteLong(writer, s.AreaLightRebuiltCommandBuffers);
                writer.Write(",\"remoteBudgetedCharacterCount\":"); WriteInt(writer, s.RemoteBudgetedCharacterCount);
                writer.Write(",\"remoteCulledAnimatorCount\":"); WriteInt(writer, s.RemoteCulledAnimatorCount);
                writer.Write(",\"remoteSkippedPropUpdates\":"); WriteLong(writer, s.RemoteSkippedPropUpdates);
                writer.Write(",\"remoteSkippedTriggerSearches\":"); WriteLong(writer, s.RemoteSkippedTriggerSearches);
                writer.Write(",\"remoteSkippedPresentationUpdates\":"); WriteLong(writer, s.RemoteSkippedPresentationUpdates);
                writer.Write(",\"remoteCombatShots\":"); WriteLong(writer, s.RemoteCombatShots);
                writer.Write(",\"remoteSoundOnlyShots\":"); WriteLong(writer, s.RemoteSoundOnlyShots);
                writer.Write(",\"remoteCombatSafetyBypasses\":"); WriteLong(writer, s.RemoteCombatSafetyBypasses);
                writer.Write(",\"remoteCulledMuzzleEffects\":"); WriteLong(writer, s.RemoteCulledMuzzleEffects);
                writer.Write(",\"remoteCulledImpactEffects\":"); WriteLong(writer, s.RemoteCulledImpactEffects);
                writer.Write(",\"remoteCulledCasings\":"); WriteLong(writer, s.RemoteCulledCasings);
                writer.Write(",\"remoteCulledLights\":"); WriteInt(writer, s.RemoteCulledLights);
                writer.Write(",\"remoteSoundOnlyAuthority\":"); writer.Write(s.RemoteSoundOnlyAuthority ? "true" : "false");
                writer.Write(",\"remoteCombatDecisionMs\":"); WriteDouble(writer, s.RemoteCombatDecisionMs);
                ...
                writer.Write(",\"pipRenderingDisabled\":"); writer.Write(s.PipRenderingDisabled ? "true" : "false");
                writer.Write(",\"pipScopeSourceResolution\":"); WriteInt(writer, s.PipScopeSourceResolution);
                writer.Write(",\"pipScopeOptimizedResolution\":"); WriteInt(writer, s.PipScopeOptimizedResolution);
                writer.Write(",\"pipScopeRenderedFrames\":"); WriteLong(writer, s.PipScopeRenderedFrames);
                writer.Write(",\"pipScopeReusedFrames\":"); WriteLong(writer, s.PipScopeReusedFrames);
                writer.Write(",\"pipScopeAverageRenderMs\":"); WriteDouble(writer, s.PipScopeAverageRenderMs);
                writer.Write(",\"compatibilityFastWorldLookups\":"); WriteLong(writer, s.CompatibilityFastWorldLookups);
        ...
        private static void WriteDouble(TextWriter writer, double value)
        {
            ...
            if (!value.TryFormat(buffer, out int written, "0.######", Invariant)) throw new InvalidOperationException("Unable to format benchmark value.");
            writer.Write(buffer.Slice(0, written));
        }
        private static void WriteInt(TextWriter writer, int value)
        {
            Span<char> buffer = stackalloc char[16];
            if (!value.TryFormat(buffer, out int written, default, Invariant)) throw new InvalidOperationException("Unable to format benchmark integer.");
            writer.Write(buffer.Slice(0, written));
        }
        ...
        private static void WriteNullableLong(TextWriter writer, long? value) { if (value.HasValue) WriteLong(writer, value.Value); }
        ...
        private static void WriteJsonNullable(TextWriter writer, long? value) { if (value.HasValue) WriteLong(writer, value.Value); else writer.Write("null"); }
        private static void WriteJsonNullable(TextWriter writer, int? value) { if (value.HasValue) WriteInt(writer, value.Value); else writer.Write("null"); }
