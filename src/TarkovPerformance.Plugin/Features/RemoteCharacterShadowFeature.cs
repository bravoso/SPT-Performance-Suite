...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    ...
    internal sealed class RemoteCharacterShadowFeature : IPerformanceFeature
    {
        ...
        internal RemoteCharacterShadowFeature(ManualLogSource logger, PluginConfiguration configuration, EntityRegistry registry, RecentExceptionLog exceptions)
        {
            _logger = logger;
        ...
        {
            if (!_originalStates.TryGetValue(renderer, out ShadowCastingMode original)) return;
            if (renderer != null) renderer.shadowCastingMode = original;
