...
/// <summary>Applies and restores the preset's global Unity quality settings at raid boundaries.</summary>
internal sealed class AggressiveQualityFeature : IPerformanceFeature
{
    ...
    public string Name
    {
        get { return "Aggressive Global Render Profile"; }
    }
    ...
    private void InstallAmbientCommandPatch()
    {
        try
        {
            ...
            if (_ambientCommandRebuild == null || prefix == null)
            {
                throw new MissingMethodException("AmbientLight command-buffer rebuild method was not found.");
            }
    ...
    {
        return _instance == null || _instance.ShouldRebuildAmbientCommands(__instance);
    }

    private bool ShouldRebuildAmbientCommands(AmbientLight light)
    {
        if (!IsEnabled || light == null || !light.IsInitialized || _ambientCommandRebuild == null)
        {
            return true;
        }
    ...
    private void Apply()
    {
        try
        {
            ...
            _logger.LogWarning(
                Name
                    + " applied. This deliberately reduces visual quality; change texture mip limits before loading a raid whenever possible."
            );
        }
        catch (Exception ex)
        {
    ...
    private void Restore()
    {
        ...
        foreach (KeyValuePair<AmbientLight, float> pair in _ambientReflectionDelays)
        {
            if (pair.Key != null)
            {
                pair.Key.RenderDelay = pair.Value;
            }
        }
    ...
    private static int ValidShadowCascades(int value)
    {
        return value >= 4 ? 4
            : value >= 2 ? 2
            : 0;
    ...
    private float ValidatedAmbientRate()
    {
        return Clamp(_configuration.AggressiveAmbientReflectionRate.Value, 5f, 30f);
    }

    private float ValidatedAmbientCommandRate()
    {
        return Clamp(_configuration.AggressiveAmbientCommandRate.Value, 8f, 60f);
    }
    ...
    private void ApplyAmbientReflectionBudget()
    {
        ...
        for (int i = 0; i < lights.Length; i++)
        {
            ...
            if (!_ambientReflectionDelays.ContainsKey(light))
            {
                _ambientReflectionDelays.Add(light, light.RenderDelay);
            }

            if (light.RenderDelay < minimumDelay)
            {
                light.RenderDelay = minimumDelay;
            }
        }
    }

    /// <summary>Captures every global quality value changed by this feature for exact restoration.</summary>
    private readonly struct QualitySnapshot
    {
        ...
        internal static QualitySnapshot Capture()
        {
