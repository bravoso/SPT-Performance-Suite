...
/// <summary>Suppresses known repetitive combat log messages while leaving warnings and errors untouched.</summary>
internal sealed class HotPathLogSuppressionFeature : IPerformanceFeature
{
    ...
    public string Name
    {
        get { return "Known Combat Log Suppression"; }
    }
    ...
    internal string StatusText
    {
        get
        {
            return IsEnabled ? "enabled | exact RealisticFrag projectile logs suppressed " + _suppressed
    ...
    public void Initialize()
    {
        ...
        try
        {
            ...
            {
                throw new MissingMethodException("BepInEx ManualLogSource.Log was not found.");
            }
    ...
    {
        if (enabled && (_breaker.IsOpen || _target == null))
        {
            return;
        }
    ...
    public void Shutdown()
    {
        _raidActive = false;
        IsEnabled = false;
        if (_target != null && _harmony != null)
        {
            _harmony.Unpatch(_target, HarmonyPatchType.Prefix, _harmony.Id);
        }
    ...
    {
        if (!IsEnabled || !_raidActive || source == null)
        {
            return true;
        }

        if ((level & (LogLevel.Fatal | LogLevel.Error | LogLevel.Warning)) != 0)
        {
            return true;
        }

        if (!string.Equals(source.SourceName, "RealisticFrag.Client", StringComparison.Ordinal))
        {
            return true;
        }

        if (!(data is string message) || !message.StartsWith("[RealisticFrag] gated frag for ammo ", StringComparison.Ordinal))
        {
            return true;
        }
    ...
    {
        return _instance == null || _instance.ShouldLog(__instance, level, data);
    }
}
