...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    ...
    internal sealed class WorldPresentationRateFeature : IPerformanceFeature
    {
        ...
        public string Name => "World Presentation CPU Budget";
        public bool IsAvailable => _patchesInstalled;
        public bool IsEnabled { get; private set; }
        internal string StatusText => !_patchesInstalled ? "unavailable"
            : !IsEnabled ? "disabled"
            : "enabled | culling " + Rate(_configuration.CullingRefreshRate.Value, 20f, 120f, 30f).ToString("F0") + " Hz"
                + " shadows " + Rate(_configuration.DistantShadowRefreshRate.Value, 5f, 60f, 15f).ToString("F0") + " Hz"
                + " decals " + Rate(_configuration.DeferredDecalRefreshRate.Value, 5f, 60f, 15f).ToString("F0") + " Hz"
                + " weather " + Rate(_configuration.WeatherRefreshRate.Value, 5f, 60f, 10f).ToString("F0") + " Hz"
                + " | skipped " + (_skippedCulling + _skippedDistantShadow + _skippedDecal + _skippedWeather);
        ...
        public void Shutdown()
        {
            ...
            if (_harmony != null)
            {
                if (_cullingUpdate != null) _harmony.Unpatch(_cullingUpdate, HarmonyPatchType.Prefix, _harmony.Id);
                if (_distantShadowUpdate != null) _harmony.Unpatch(_distantShadowUpdate, HarmonyPatchType.Prefix, _harmony.Id);
                if (_decalUpdate != null) _harmony.Unpatch(_decalUpdate, HarmonyPatchType.Prefix, _harmony.Id);
                if (_weatherUpdate != null) _harmony.Unpatch(_weatherUpdate, HarmonyPatchType.Prefix, _harmony.Id);
            }
        ...
        private void InstallPatches()
        {
            try
            {
                ...
                _harmony.Patch(_cullingUpdate, prefix: new HarmonyMethod(AccessTools.Method(typeof(WorldPresentationRateFeature), nameof(CullingPrefix))));
                _harmony.Patch(_distantShadowUpdate, prefix: new HarmonyMethod(AccessTools.Method(typeof(WorldPresentationRateFeature), nameof(DistantShadowPrefix))));
                _harmony.Patch(_decalUpdate, prefix: new HarmonyMethod(AccessTools.Method(typeof(WorldPresentationRateFeature), nameof(DecalPrefix))));
                _harmony.Patch(_weatherUpdate, prefix: new HarmonyMethod(AccessTools.Method(typeof(WorldPresentationRateFeature), nameof(WeatherPrefix))));
                _patchesInstalled = true;
                _logger.LogWarning(Name + " active: global culling and visual maintenance use independent rate caps; simulation and networking remain untouched.");
            }
            catch (Exception ex)
            {
                _patchesInstalled = false;
        ...
        {
            return assembly.GetType(typeName, false)?.GetMethod(methodName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, Type.EmptyTypes, null);
        }

        private static bool CullingPrefix()
        {
            WorldPresentationRateFeature feature = _instance;
            return feature == null || feature.ShouldRun(ref feature._nextCulling, feature._configuration.CullingRefreshRate.Value, 20f, 120f, 30f, ref feature._skippedCulling);
        }

        private static bool DistantShadowPrefix()
        {
            WorldPresentationRateFeature feature = _instance;
            return feature == null || feature.ShouldRun(ref feature._nextDistantShadow, feature._configuration.DistantShadowRefreshRate.Value, 5f, 60f, 15f, ref feature._skippedDistantShadow);
        }

        private static bool DecalPrefix()
        {
            WorldPresentationRateFeature feature = _instance;
            return feature == null || feature.ShouldRun(ref feature._nextDecal, feature._configuration.DeferredDecalRefreshRate.Value, 5f, 60f, 15f, ref feature._skippedDecal);
        }

        private static bool WeatherPrefix()
        {
            WorldPresentationRateFeature feature = _instance;
            return feature == null || feature.ShouldRun(ref feature._nextWeather, feature._configuration.WeatherRefreshRate.Value, 5f, 60f, 10f, ref feature._skippedWeather);
        }

        private bool ShouldRun(ref float next, float configuredRate, float minimum, float maximum, float fallback, ref long skipped)
        {
            if (!IsEnabled || !_raidActive) return true;
        ...
        {
            return float.IsNaN(value) || float.IsInfinity(value) ? fallback : value < minimum ? minimum : value > maximum ? maximum : value;
        }
    }
}
