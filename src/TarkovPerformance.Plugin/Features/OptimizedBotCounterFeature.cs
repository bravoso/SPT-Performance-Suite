...
/// <summary>Contains the bot, boss, and special-faction counts shown by the independent HUD.</summary>
...
/// <summary>Maintains a low-frequency bot counter from the shared entity registry without scanning every frame.</summary>
internal sealed class OptimizedBotCounterFeature
{
    ...
    internal OptimizedBotCounts Counts
    {
        get { return _counts; }
    }
    ...
    internal string StatusText
    {
        get
        {
            return _configuration.BotCounterEnabled.Value
    ...
    {
        if (_configuration.BotCounterEnabled.Value)
        {
    ...
    {
        if (!_raidActive || !_configuration.BotCounterEnabled.Value || now < _nextRefresh)
        {
            return;
        }
        ...
        foreach (TrackedEntity entity in _entities.Entities)
        {
            ...
            if (player == null || entity.Kind != Features.EntityKind.RemoteAI || !entity.IsAlive)
            {
                continue;
            }
            ...
            if (IsBlackDivision(role, roleName))
            {
                blackDivision++;
                continue;
            }
            if (IsRuafRemnant(role, roleName))
            {
                ruafRemnant++;
                continue;
            }
            if (IsRuaf(role, roleName))
            {
                ruaf++;
                continue;
            }
            if (IsUntar(role, roleName))
            {
                untar++;
                continue;
            }
            ...
            {
                boss++;
                if (NamedBossIndex.TryGetValue(role, out int index))
                {
                    _workingBossNameCounts[index]++;
                }
            }
            else
            {
    ...
    internal void Draw()
    {
        if (!_raidActive || !_configuration.BotCounterEnabled.Value)
        {
            return;
        }
    ...
    private void RefreshStyle()
    {
        if (_style == null)
        {
            _style = new GUIStyle { alignment = TextAnchor.UpperRight };
        }
        ...
        if (string.Equals(requested, _loadedFontName, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
        ...
        try
        {
            ...
            if (_customFont == null)
            {
                _logger.LogWarning("Bot counter font '" + requested + "' was not found; using EFT's default font.");
            }
        }
        catch (Exception ex)
        {
    ...
    private void DisableLegacyCounterIfPresent()
    {
        ...
        foreach (KeyValuePair<string, BepInEx.PluginInfo> pair in Chainloader.PluginInfos)
        {
            ...
            if (!string.Equals(info?.Metadata?.GUID, "com.yourname.botcounter", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            if (info.Instance is BaseUnityPlugin plugin)
            {
                ...
                if (plugin.Config.TryGetEntry(definition, out ConfigEntry<bool> enabled))
                {
                    enabled.Value = false;
                }
            }
            ...
            _logger.LogWarning(
                "Disabled the legacy SPT Detailed Bot Counter because the suite now provides a cached replacement without periodic scans."
            );
            return;
        }
    }
    ...
    private void UpdateLines(OptimizedBotCounts counts)
    {
        ...
        {
            AddCountLine("Bosses & Guards", counts.Boss, hideZero);
        }
    ...
    {
        return BlackDivisionRoles.Contains(role) || StartsWith(name, "blackDiv");
    }

    private static bool IsRuafRemnant(int role, string name)
    {
        return RuafRemnantRoles.Contains(role) || StartsWith(name, "remnant");
    }

    private static bool IsRuaf(int role, string name)
    {
        return RuafRoles.Contains(role) || StartsWith(name, "ruaf");
    }

    private static bool IsUntar(int role, string name)
    {
        return UntarRoles.Contains(role) || name.IndexOf("untar", StringComparison.OrdinalIgnoreCase) >= 0;
    }
