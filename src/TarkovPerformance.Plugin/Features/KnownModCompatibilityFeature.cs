...
/// <summary>Detects optional mods and applies only compatibility changes verified for their public APIs.</summary>
internal sealed class KnownModCompatibilityFeature : IPerformanceFeature
{
    ...
    public string Name
    {
        get { return "Known Mod CPU Compatibility"; }
    }
    ...
    internal long FastLookups
    {
        get { return _fastLookups; }
    }
    ...
    internal string StatusText
    {
        get
        {
            ...
            if (_botCounterTarget == null)
            {
                return _discoveryComplete ? "enabled | Bot Counter not installed" : "enabled | checking installed mods";
            }
    ...
    {
        if (!IsEnabled || _discoveryComplete || _botCounterTarget != null || now < _nextDiscovery)
        {
            return;
        }
    ...
    {
        if (!IsEnabled || _botCounterTarget != null)
        {
            return;
        }
        ...
        try
        {
            ...
            {
                if (!string.Equals(pair.Value?.Metadata?.GUID, "com.yourname.botcounter", StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
            ...
            {
                throw new MissingMethodException("SPT Detailed Bot Counter 1.7 update method was not found.");
            }
            ...
            _breaker.Success();
            _logger.LogWarning(
                "Removed SPT Detailed Bot Counter's repeating Object.FindObjectOfType<GameWorld>() scan. Counts and UI remain unchanged; the suite supplies its cached raid world."
            );
        }
        catch (Exception ex)
        {
