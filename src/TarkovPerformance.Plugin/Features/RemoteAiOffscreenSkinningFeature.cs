...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    ...
    internal sealed class RemoteAiOffscreenSkinningFeature : IPerformanceFeature
    {
        ...
        internal RemoteAiOffscreenSkinningFeature(ManualLogSource logger, PluginConfiguration configuration, EntityRegistry registry, RecentExceptionLog exceptions)
        {
            _logger = logger;
        ...
        internal void Tick(float now)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_breaker.Failure())
                {
                    ...
                    RestoreAll();
                    _logger.LogError(Name + " circuit breaker activated after repeated exceptions; all original renderer states were restored.");
                }
            }
            finally
            {
        ...
        private void ProcessEntities(float now, ValidatedConfiguration configuration)
        {
            ...
            foreach (TrackedEntity entity in _registry.Entities)
            {
                ...
                if (now - since < configuration.SkinningOffscreenHoldSeconds) continue;
        ...
        {
            for (int i = 0; i < renderers.Length; i++) RestoreOne(renderers[i]);
        }
        ...
        private void RestoreNoLongerTracked()
        {
            ...
            foreach (KeyValuePair<SkinnedMeshRenderer, bool> pair in _originalStates)
                if (pair.Key == null || !_seenThisTick.Contains(pair.Key)) _restoreBuffer.Add(pair.Key);
            for (int i = 0; i < _restoreBuffer.Count; i++) RestoreOne(_restoreBuffer[i]);
        }
        ...
        private void RemoveUnseenEntities()
        {
            ...
            foreach (KeyValuePair<int, float> pair in _offscreenSince)
                if (!_seenEntities.Contains(pair.Key)) _entityRemoveBuffer.Add(pair.Key);
            for (int i = 0; i < _entityRemoveBuffer.Count; i++) _offscreenSince.Remove(_entityRemoveBuffer[i]);
        }

        private void RestoreOne(SkinnedMeshRenderer renderer)
        {
            if (ReferenceEquals(renderer, null)) return;
            if (!_originalStates.TryGetValue(renderer, out bool original)) return;
            if (renderer != null) renderer.updateWhenOffscreen = original;
        ...
        {
            foreach (KeyValuePair<SkinnedMeshRenderer, bool> pair in _originalStates)
                if (pair.Key != null) pair.Key.updateWhenOffscreen = pair.Value;
