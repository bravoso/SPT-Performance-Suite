...
/// <summary>Caches one player entity and the visibility/classification data shared by runtime features.</summary>
internal sealed class TrackedEntity
{
    internal Player Player;

    // RemoteAI is intentionally a broad safety class. It currently includes bosses and special factions;
    // role-specific names are resolved separately by the HUD and are not a universal optimization exemption.
...
/// <summary>Contains a point-in-time count of tracked entities by safety classification.</summary>
internal readonly struct EntityCounts
{
    internal EntityCounts(
        int players,
        int local,
        int remoteHuman,
        int ai,
        int livingAi,
        int visibleAi,
        int bakedCullingEntities,
        int bakedHiddenEntities,
        int corpses,
        int animators,
        int skinned,
        int shadow
    )
    {
        Players = players;
        LocalPlayers = local;
        RemoteHumans = remoteHuman;
        Ai = ai;
        LivingAi = livingAi;
        VisibleAi = visibleAi;
        BakedCullingEntities = bakedCullingEntities;
        BakedHiddenEntities = bakedHiddenEntities;
        Corpses = corpses;
        Animators = animators;
        SkinnedRenderers = skinned;
        ShadowRenderers = shadow;
    }
...
/// <summary>Refreshes and classifies EFT players at a bounded rate for all presentation features.</summary>
internal sealed class EntityRegistry
{
    ...
    internal IEnumerable<TrackedEntity> Entities
    {
    ...
    {
        if (_world == null || _world.RegisteredPlayers == null)
        {
            return;
        }
        ...
        if (_cursor != 0 || now >= _nextCycle)
        {
            ...
            {
                if (_world.RegisteredPlayers[_cursor] is Player player && player != null)
                {
                    RegisterOrRefresh(player, now);
                }
    ...
    internal EntityCounts CountNow(float now)
    {
        ...
        foreach (TrackedEntity entity in _entities.Values)
        {
            ...
            else if (entity.Kind == EntityKind.RemoteHuman)
            {
                humans++;
            }
            ...
            {
                baked++;
                if (!entity.IsBakedCullingVisible)
                {
                    bakedHidden++;
                }
            }
            ...
            {
                if (entity.Animators[i] != null && entity.Animators[i].isActiveAndEnabled)
                {
                    animators++;
                }
            }
            ...
            {
                if (
                    entity.SkinnedRenderers[i] != null
                    && entity.SkinnedRenderers[i].enabled
                    && entity.SkinnedRenderers[i].gameObject.activeInHierarchy
                )
                {
                    skinned++;
                }
        ...
        _cachedCounts = new EntityCounts(
            players,
            local,
            humans,
            ai,
    ...
    private void RegisterOrRefresh(Player player, float now)
    {
        ...
        if (now >= entity.NextComponentRefresh)
        {
            ...
            {
                if (entity.RendererBuffer[i] is SkinnedMeshRenderer skinned)
                {
                    entity.SkinnedBuffer.Add(skinned);
                }
            }
    ...
    private void RefreshSnapshots()
    {
        ...
        foreach (TrackedEntity entity in _entities.Values)
        {
            ...
            if (visible)
            {
                entity.LastVisibleTime = Time.realtimeSinceStartup;
            }
        }
    }
    ...
    private void RemoveUnseen()
    {
        ...
        {
            if (pair.Value.Player == null || pair.Value.SeenGeneration != _generation)
            {
        ...
        {
            _entities.Remove(_removeBuffer[i]);
        }
    }
...
/// <summary>Queries EFT's baked occlusion system defensively across supported game builds.</summary>
internal static class EftBakedCullingAdapter
{
    private static readonly FieldInfo CullingHandlerField = typeof(LocalPlayer).GetField(
        "localPlayerCullingHandlerClass",
        BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic
    );

    internal static bool IsAvailable
    {
        get { return CullingHandlerField != null; }
    }

    internal static bool TryRead(Player player, out bool visible)
    {
        visible = true;
        if (CullingHandlerField == null || !(player is LocalPlayer localPlayer))
        {
            return false;
        }
        ...
        {
            if (
                !(CullingHandlerField.GetValue(localPlayer) is LocalPlayerCullingHandlerClass handler)
...
/// <summary>Converts live EFT/Fika player signals into the dependency-free core classification model.</summary>
