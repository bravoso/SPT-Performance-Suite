...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    internal sealed class DynamicMapsCompatibilityFeature : IPerformanceFeature
    {
        ...
        public string Name => "Dynamic Maps CPU Budget";
        ...
        internal string StatusText
        {
            get
            {
                if (!_patchInstalled) return "not installed or incompatible";
                if (!IsEnabled) return _breaker.IsOpen ? "disabled (circuit breaker)" : "disabled";
                return "enabled | minimap recenter " + Clamp(_configuration.DynamicMapsMiniMapRefreshRate.Value, 15f, 60f).ToString("F0")
        ...
        public void OnRaidStarted()
        {
            ...
            if (IsEnabled) ApplyLeanMarkerConfiguration();
        }
        ...
        {
            if (_patchInstalled || now < _nextDiscovery) return;
        ...
        {
            if (enabled && (!IsAvailable || _breaker.IsOpen)) return;
            IsEnabled = enabled;
            _configuration.DynamicMapsOptimizationEnabled.Value = enabled;
            _nextMiniMapCenter = 0;
            if (enabled) ApplyLeanMarkerConfiguration();
        }

        public void Shutdown()
        {
            IsEnabled = false;
            if (_harmony != null && _precacheCoroutine != null) _harmony.Unpatch(_precacheCoroutine, HarmonyPatchType.Prefix, _harmony.Id);
            if (_harmony != null && _onCenter != null) _harmony.Unpatch(_onCenter, HarmonyPatchType.Prefix, _harmony.Id);
        ...
        private void DiscoverAndPatch()
        {
            ...
            try
            {
                ...
                MethodInfo showingMiniMapGetter = mapScreen?.GetProperty("_showingMiniMap", BindingFlags.Instance | BindingFlags.NonPublic)?.GetGetMethod(true);
                ...
                if (_dynamicMapsPlugin == null || _precacheCoroutine == null || _onCenter == null || showingMiniMapGetter == null
                    || precachePrefix == null || miniMapCenterPrefix == null)
                {
                    _patchInstalled = false;
                    return;
                }
                ...
                _logger.LogWarning("Dynamic Maps compatibility active: minimap recentering is rate-limited without throttling map input; all-map image preloading is blocked and marker providers are reduced to the requested set.");
            }
            catch (Exception ex)
            {
                _patchInstalled = false;
        ...
        {
            DynamicMapsCompatibilityFeature instance = _instance;
            if (instance == null || !instance.IsEnabled || instance._isShowingMiniMap == null) return true;
            try
            {
                ...
                if (!instance._isShowingMiniMap(__instance)) return true;
                float now = Time.realtimeSinceStartup;
                if (now < instance._nextMiniMapCenter) return false;
        ...
        private void RecordFailure(string operation, Exception ex)
        {
            ...
            {
                IsEnabled = false;
                _logger.LogWarning(Name + " disabled after repeated " + operation + " failures; Dynamic Maps now runs unchanged: " + ex.Message);
            }
        }

        private void ApplyLeanMarkerConfiguration()
        {
            if (!_configuration.DynamicMapsLeanMarkers.Value || _dynamicMapsPlugin == null) return;
            try
            {
        ...
        {
            if (config.TryGetEntry(new ConfigDefinition(section, key), out ConfigEntry<bool> entry) && entry.Value != value) entry.Value = value;
        }
        ...
        private static BepInEx.PluginInfo FindPlugin(string guid)
        {
            foreach (KeyValuePair<string, BepInEx.PluginInfo> pair in Chainloader.PluginInfos)
                if (string.Equals(pair.Value?.Metadata?.GUID, guid, StringComparison.OrdinalIgnoreCase)) return pair.Value;
            return null;
        }
