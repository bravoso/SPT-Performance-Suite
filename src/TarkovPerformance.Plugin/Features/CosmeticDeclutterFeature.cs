...
/// <summary>Reports reversible cosmetic objects disabled by the declutter feature.</summary>
...
/// <summary>Disables selected non-gameplay scene decorations and restores their original active state.</summary>
internal sealed class CosmeticDeclutterFeature : IPerformanceFeature
{
    private static readonly string[] IncludeTokens =
    {
        "decal",
        "trash",
        "garbage",
    ...
    private static readonly string[] ExcludeTokens =
    {
        "player",
        "bot",
        "weapon",
    ...
    internal DeclutterCounters Counters
    {
    ...
    {
        _breaker.Reset();
        SetEnabled(_configuration.CosmeticDeclutterEnabled.Value);
    }
    ...
    private void Discover()
    {
        ...
        _logger.LogInfo(
            Name
                ...
                + _scan.Length
                + " renderers in "
                + _discoveryMs.ToString("F1")
                + " ms; classification is amortized across frames."
        );
    }
    ...
    private static bool IsCandidate(Renderer renderer)
    {
        ...
        {
            if (identity.Contains(ExcludeTokens[i]))
            {
                return false;
            }
        }
        ...
        bool included = false;
        for (int i = 0; i < IncludeTokens.Length; i++)
        {
            if (identity.Contains(IncludeTokens[i]))
            {
                included = true;
                break;
            }
    ...
    private static string HierarchyIdentity(Transform transform)
    {
        ...
        for (int i = 0; i < 4 && transform != null; i++, transform = transform.parent)
        {
            value += "|" + transform.name.ToLowerInvariant();
        }
    ...
    {
        foreach (KeyValuePair<Renderer, bool> item in _originalStates)
        {
            if (item.Key != null)
            {
