...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    ...
    internal sealed class AreaLightCommandCacheFeature : IPerformanceFeature
    {
        ...
        {
            internal RefreshEntry(int nextFrame, int stateHash) { NextFrame = nextFrame; StateHash = stateHash; }
        ...
        public string Name => "Static Area-Light Command Cache";
        public bool IsAvailable => _patchesInstalled && !_breaker.IsOpen;
        public bool IsEnabled { get; private set; }
        internal AreaLightCacheCounters Counters => new AreaLightCacheCounters(
            Interlocked.Read(ref _reused), Interlocked.Read(ref _rebuilt), Interlocked.Read(ref _shadowBypasses));
        internal string StatusText => IsEnabled
            ? "enabled | reused " + Interlocked.Read(ref _reused) + " rebuilt " + Interlocked.Read(ref _rebuilt) + " shadow-safe bypass " + Interlocked.Read(ref _shadowBypasses)
            : _breaker.IsOpen ? "disabled (circuit breaker)" : _patchesInstalled ? "disabled" : "unavailable";
        ...
        public void OnRaidEnded() => _states = new ConditionalWeakTable<AreaLight, CameraRefreshState>();
        ...
        public void SetEnabled(bool enabled)
        {
            ...
            _states = new ConditionalWeakTable<AreaLight, CameraRefreshState>();
            _logger.LogInfo(Name + " " + (desired ? "enabled" : "disabled") + ". Shadowed and animated area lights always retain vanilla updates.");
        }
        ...
        {
            if (!IsEnabled || light == null || camera == null) return true;
        ...
        {
            if (!IsEnabled || light == null || camera == null || light.m_Shadows || light.IsShadowCubeAnimated) return;
            int interval = Clamp(_configuration.AreaLightRefreshFrames.Value, 1, 8);
            CameraRefreshState state = _states.GetOrCreateValue(light);
            int cameraId = camera.GetInstanceID();
        ...
        {
            AreaLightCommandCacheFeature instance = _instance;
            if (!__state || instance == null) return;
            try { instance.MarkBuilt(__instance, cam); }
            catch (Exception ex) { instance.Failure(ex); }
        }

        private static void InvalidatePostfix(AreaLight __instance)
        {
            if (__instance != null) _states.Remove(__instance);
        }
