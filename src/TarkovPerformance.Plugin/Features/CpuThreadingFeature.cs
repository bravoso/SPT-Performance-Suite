...
namespace TarkovPerformanceSuite.RuntimeFeatures
{
    internal sealed class CpuThreadingFeature : IPerformanceFeature
    {
        ...
        public string Name => "CPU Logical Processor Access";
        public bool IsAvailable => System.Runtime.InteropServices.RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && !_breaker.IsOpen;
        public bool IsEnabled { get; private set; }
        internal string StatusText
        {
            get
            {
                if (_breaker.IsOpen) return "disabled (circuit breaker)";
                if (!IsAvailable) return "unavailable on this platform";
            ...
            try
            {
                using (Process process = Process.GetCurrentProcess())
                {
                    if (!_haveOriginal)
                    {
                        ...
                        _haveOriginal = true;
                        _logger.LogInfo($"CPU affinity experiment: original mask exposes {CountBits(_originalAffinity)} logical processors; all-logical mask exposes {CountBits(_targetAffinity)}.");
                    }
        ...
        private void Restore()
        {
            ...
            try
            {
                using (Process process = Process.GetCurrentProcess()) process.ProcessorAffinity = _originalAffinity;
            }
        ...
        private static int DetectLogicalProcessorCount()
        {
            try
            {
                ...
                if (count > 0) return (int)Math.Min(count, IntPtr.Size * 8);
            }
            catch { }
        ...
        private static int ReadStaticInt(Type type, string name)
        {
            ...
            if (property != null && property.PropertyType == typeof(int)) return (int)property.GetValue(null, null);
