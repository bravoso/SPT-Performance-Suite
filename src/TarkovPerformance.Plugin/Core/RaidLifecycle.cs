...
namespace TarkovPerformanceSuite.Core
{
    ...
    internal sealed class RaidLifecycle
    {
        ...
        internal void Tick(float now)
        {
            ...
            {
                if (State == RaidState.Started) RaidEnded?.Invoke(_world);
                _world = null;
        ...
        {
            if (State == RaidState.Started && !ReferenceEquals(_world, null)) RaidEnded?.Invoke(_world);
            _world = null;
            ChangeState(RaidState.MainMenu);
        }
        ...
        {
            if (world == null || world.RegisteredPlayers == null) return false;
