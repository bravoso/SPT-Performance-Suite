...
namespace TarkovPerformanceSuite.Core
{
    internal sealed class RuntimeInformation
    {
        ...
        internal bool FikaDetected => !string.IsNullOrEmpty(FikaVersion);
        ...
        internal static RuntimeInformation Detect()
        {
            ...
            if (string.IsNullOrEmpty(info.EftVersion)) info.EftVersion = Application.version;
            if (string.IsNullOrEmpty(info.SptVersion)) info.SptVersion = "not detected";
            return info;
        }

        internal void Log(ManualLogSource logger)
        {
            logger.LogInfo($"Environment: EFT {EftVersion}; Unity {UnityVersion}; SPT {SptVersion}; BepInEx {BepInExVersion}; Fika {(FikaDetected ? FikaVersion : "not installed/detected")}");
        }
    ...
    internal sealed class RecentExceptionLog
    {
        ...
        {
            if (_count == 0) { builder.AppendLine("(none)"); return; }
            int start = (_next - _count + _entries.Length) % _entries.Length;
            for (int i = 0; i < _count; i++) builder.AppendLine(_entries[(start + i) % _entries.Length]);
        }
    }
}
