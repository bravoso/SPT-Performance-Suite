...
namespace TarkovPerformanceSuite.RuntimeDiagnostics
{
    ...
    {
        internal static string Write(string directory, RuntimeInformation runtime, ProfilerMetrics metrics, EntityCounts counts, RecentExceptionLog exceptions, string featureState, string benchmarkConfiguration, string methodPatchReport, double suiteMs)
        {
            Directory.CreateDirectory(directory);
            string path = Path.Combine(directory, "diagnostics_" + DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss") + ".txt");
            var builder = new StringBuilder(8192);
            builder.AppendLine("Tarkov Performance Suite diagnostics 0.1.0");
            builder.AppendLine("Generated: " + DateTime.Now.ToString("O"));
            builder.AppendLine();
            builder.AppendLine("VERSIONS");
            builder.AppendLine($"EFT={runtime.EftVersion}; Unity={runtime.UnityVersion}; SPT={runtime.SptVersion}; BepInEx={runtime.BepInExVersion}; Fika={(runtime.FikaDetected ? runtime.FikaVersion : "not detected")}");
            builder.AppendLine();
            ...
            try
            {
                ...
                for (int i = 0; i < files.Length; i++) builder.AppendLine(Path.GetFileName(files[i]));
            }
            catch (Exception ex) { builder.AppendLine("unavailable: " + ex.Message); }
            builder.AppendLine();
            builder.AppendLine("TARGET PATCH OWNERS");
            builder.AppendLine(string.IsNullOrEmpty(methodPatchReport) ? "Method timing disabled; owners will be inspected when targets initialize." : methodPatchReport);
            builder.AppendLine();
            ...
            for (int i = 0; i < available.Count; i++) builder.AppendLine(available[i]);
            builder.AppendLine();
