...
namespace TarkovPerformanceSuite.RuntimeDiagnostics
{
    internal sealed class ProcessThreadSampler
    {
        ...
        {
            try { _mainOsThreadId = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? unchecked((int)GetCurrentThreadId()) : -1; }
            catch { _mainOsThreadId = -1; }
        }
        ...
        internal string BuildReport()
        {
            ...
            builder.AppendLine("Logical processors: " + _logicalProcessors + " | samples: " + _sampleCount + " | observed wall time: " + _observedWallSeconds.ToString("F2", CultureInfo.InvariantCulture) + " s");
            double oneCoreScale = WholeProcessPercent();
            builder.AppendLine("CPU time observed: " + _totalCpuMilliseconds.ToString("F2", CultureInfo.InvariantCulture) + " ms | core-equivalent CPU: " + oneCoreScale.ToString("F1", CultureInfo.InvariantCulture) + "% | Task-Manager-style CPU: " + (oneCoreScale / _logicalProcessors).ToString("F1", CultureInfo.InvariantCulture) + "%");
            builder.AppendLine("A thread near 100% of one core is saturated. Core-equivalent process CPU may exceed 100% when multiple cores are busy.");
            builder.AppendLine("Native thread reads: " + _successfulReads + " successful | " + _failedReads + " failed");
            if (!string.IsNullOrWhiteSpace(_lastError)) builder.AppendLine("Sampler warning: " + _lastError);
            ...
            for (int i = 0; i < shown; i++)
            {
                ...
                if (sample.AccumulatedCpuMs <= 0) continue;
                double oneCore = _observedWallSeconds > 0 ? sample.AccumulatedCpuMs / (_observedWallSeconds * 10.0) : 0;
                builder.Append(i + 1).Append(" | ").Append(sample.Id).Append(" | ")
                    .Append(sample.Id == _mainOsThreadId ? "Unity main" : "worker/native")
                    .Append(" | ").Append(sample.AccumulatedCpuMs.ToString("F2", CultureInfo.InvariantCulture))
                    .Append(" | ").Append(oneCore.ToString("F1", CultureInfo.InvariantCulture))
                    .Append(" | ").Append(sample.PeakOneCorePercent.ToString("F1", CultureInfo.InvariantCulture))
                    .Append(" | ").Append(sample.Intervals).AppendLine();
            }
                    ...
                    foreach (ProcessThread thread in process.Threads)
                    {
                        try
                        {
                            ...
                            if (!baselineOnly && delta >= 0 && delta < 60000)
                            {
                                ...
                                if (intervalPercent > sample.PeakOneCorePercent) sample.PeakOneCorePercent = intervalPercent;
                        ...
                        {
                            _failedReads++;
                            if (string.IsNullOrWhiteSpace(_lastError)) _lastError = ex.GetType().Name + ": " + ex.Message;
                        }
                    }
                }
                if (!baselineOnly)
        ...
        private double WholeProcessPercent() => _observedWallSeconds > 0 ? _totalCpuMilliseconds / (_observedWallSeconds * 10.0) : 0;

        private static double ReadThreadCpuMilliseconds(ProcessThread thread)
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return thread.TotalProcessorTime.TotalMilliseconds;
            IntPtr handle = OpenThread(ThreadQueryLimitedInformation, false, unchecked((uint)thread.Id));
            if (handle == IntPtr.Zero) throw new InvalidOperationException("OpenThread failed for " + thread.Id + ".");
            try
            {
                if (!GetThreadTimes(handle, out NativeFileTime created, out NativeFileTime exited, out NativeFileTime kernel, out NativeFileTime user))
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern bool GetThreadTimes(IntPtr threadHandle, out NativeFileTime creationTime, out NativeFileTime exitTime, out NativeFileTime kernelTime, out NativeFileTime userTime);
