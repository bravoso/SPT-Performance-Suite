...
/// <summary>Renders the optional lightweight diagnostics panel; it is inactive unless explicitly enabled.</summary>
internal sealed class DiagnosticsOverlay
{
    ...
    internal bool NeedsRefresh(float now)
    {
        return Visible && now >= _nextTextRefresh;
    }

    internal void AddFrame(double frameTimeMs)
    {
        _rolling.Add(frameTimeMs);
    }
    ...
    internal void Refresh(
        ...
        FikaDiagnosticsAdapter fika,
        BenchmarkRecorder capture,
        string raidState,
        string experimentStatus,
        string methodText
    )
    {
        if (!Visible || now < _nextTextRefresh)
        {
            return;
        }
        ...
        _builder.Clear();
        _builder
            .Append("TARKOV PERFORMANCE SUITE ")
            .Append(Plugin.PluginVersion)
            .Append("  |  ")
            .Append(PresetName)
            .Append("  |  OPTIMIZATIONS ")
            .Append(OptimizationsActive ? "ON" : "OFF")
            .Append("  |  ")
            .AppendLine(bottleneck);
        if (DisplayMode == OverlayMode.Compact)
        {
            _builder
                ...
                .Append("   1% low ")
                .Append(onePercentLow.ToString("F0"))
                .Append("   frame ")
                .Append(instantMs.ToString("F2"))
                .AppendLine(" ms");
            _builder
                .Append("CPU main ")
                .Append(Format(mainMs))
                .Append(" ms   render ")
                .Append(Format(renderMs))
                .Append(" ms   GPU ")
                .Append(Format(gpuMs))
                .AppendLine(" ms");
            _builder
                ...
                .Append(counts.Ai)
                .Append(" (visible ")
                .Append(counts.VisibleAi)
                .Append(", map-hidden ")
                .Append(counts.BakedHiddenEntities)
                .Append(")   players ")
                .Append(counts.Players)
                .Append("   server ")
                .Append(fika.ServerFps?.ToString() ?? "n/a")
                .AppendLine(" FPS");
            _builder
                .Append("Capture: ")
                .Append(
                    capture.IsCapturing
                        ? capture.ElapsedSeconds.ToString("F0") + "/" + capture.DurationSeconds.ToString("F0") + " s"
                        : "idle"
                )
                .Append("   Suite cost ")
                .Append(SuiteAverageMs.ToString("F3"))
                .AppendLine(" ms");
        ...
        _builder
            ...
            .Append(instantMs.ToString("F2"))
            .Append(" ms | avg ")
            .Append(stats.AverageMs.ToString("F2"))
            .Append(" | median ")
            .Append(stats.MedianMs.ToString("F2"))
            .Append(" | p95 ")
            .Append(stats.P95Ms.ToString("F2"))
            .Append(" | p99 ")
            .Append(stats.P99Ms.ToString("F2"))
            .AppendLine(" ms");
        _builder
            .Append("Capture: ")
            .Append(
                capture.IsCapturing ? capture.ElapsedSeconds.ToString("F1") + "/" + capture.DurationSeconds.ToString("F0") + " s" : "idle"
            )
            .Append(" | min FPS ")
            .Append(capture.MinimumFps.ToString("F1"))
            .Append(" | avg FPS ")
            .Append(capture.AverageFps.ToString("F1"))
            .AppendLine();
        ...
        _builder.AppendLine();
        AppendMemory("managed reserve", metrics.Value("GC Reserved Memory"));
        AppendMemory("managed used", metrics.Value("GC Used Memory"));
        AppendMemory("system", metrics.Value("System Used Memory"));
        _builder.AppendLine();
        AppendLong("draw", metrics.Value("Draw Calls Count"));
        AppendLong("batches", metrics.Value("Batches Count"));
        AppendLong("SetPass", metrics.Value("SetPass Calls Count"));
        _builder.AppendLine();
        AppendLong("triangles", metrics.Value("Triangles Count"));
        AppendLong("vertices", metrics.Value("Vertices Count"));
        AppendLong("shadow casters", metrics.Value("Shadow Casters Count"));
        _builder.AppendLine();
        _builder
            .Append("Entities: players ")
            .Append(counts.Players)
            .Append(" (local ")
            .Append(counts.LocalPlayers)
            .Append(", human ")
            .Append(counts.RemoteHumans)
            .Append(") | AI ")
            .Append(counts.Ai)
            .Append(" living ")
            .Append(counts.LivingAi)
            .Append(" visible ")
            .Append(counts.VisibleAi)
            .Append(" | baked culling ")
            .Append(counts.BakedCullingEntities)
            .Append(" hidden ")
            .Append(counts.BakedHiddenEntities)
            .Append(" | observed corpses ")
            .Append(counts.Corpses)
            .AppendLine();
        _builder
            .Append("Presentation: animators ")
            .Append(counts.Animators)
            .Append(" | skinned renderers ")
            .Append(counts.SkinnedRenderers)
            .Append(" | AI shadow renderers ")
            .Append(counts.ShadowRenderers)
            .AppendLine();
        _builder
            .Append("Fika: observed ")
            .Append(fikaObserved)
            .Append(" | observed AI ")
            .Append(fikaAi)
            .Append(" | locally visible AI ")
            .Append(fikaVisibleAi)
            .Append(" | server FPS ")
            .Append(fika.ServerFps?.ToString() ?? "n/a")
            .AppendLine();
        ...
        {
            _builder.AppendLine(OptimizationSummary);
        }

        if (!string.IsNullOrEmpty(methodText))
        {
            _builder.AppendLine(methodText);
        }
    ...
    private void AppendMetric(string label, double? value, string unit)
    {
        _builder
            .Append(label)
            .Append(' ')
            .Append(value.HasValue ? value.Value.ToString("F2") : "n/a")
            .Append(' ')
            .Append(unit)
            .Append(" | ");
    }
    ...
    {
        if (!mainMs.HasValue || !gpuMs.HasValue || mainMs.Value <= 0 || gpuMs.Value <= 0)
        {
            return "MEASURING";
        }

        if (mainMs.Value > gpuMs.Value * 1.2)
        {
            return "CPU BOTTLENECK";
        }

        if (gpuMs.Value > mainMs.Value * 1.2)
        {
