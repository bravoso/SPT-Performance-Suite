...
namespace TarkovPerformanceSuite.RuntimeDiagnostics
{
    ...
    internal sealed class MethodTimingFramework
    {
        ...
        [ThreadStatic] private static ThreadTimingStack _threadStack;
        ...
        internal string GetOverlayText(double now)
        {
            ...
            if (_sortBuffer.Length < _targets.Count) Array.Resize(ref _sortBuffer, Math.Max(_targets.Count, _sortBuffer.Length * 2));
        ...
        private void AddOptionalFikaTargets()
        {
            ...
            {
                MethodInfo shotHandler = observedFirearm.GetMethod("HandleShotInfoPacket", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (shotHandler != null) AddTarget(shotHandler, "Fika.ObservedFirearm.HandleShotInfoPacket");
                else _reportBuilder.AppendLine("Fika.ObservedFirearm.HandleShotInfoPacket: expected method not found; skipped.");
            }
        }
        ...
        private void AddOptionalAiPluginTargets()
        {
            ...
            if (orbit == null) _reportBuilder.AppendLine("ORBIT: not loaded; authoritative AI targets not patched on this process.");
            else
            {
            ...
            if (sain == null) _reportBuilder.AppendLine("SAIN: not loaded; authoritative AI targets not patched on this process.");
            else
            {
                ...
                AddFirstNamedTarget(sain, "SAIN.Components.BotController.BotSpawnController", "ManualUpdate", "SAIN.SpawnController.ManualUpdate");
            ...
            if (bigBrain == null) _reportBuilder.AppendLine("BigBrain: not loaded; brain targets not patched on this process.");
            else
            {
        ...
        private void AddFirstNamedTarget(Assembly assembly, string typeName, string methodName, string displayName)
        {
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                if (!string.Equals(method.Name, methodName, StringComparison.Ordinal) || method.IsAbstract || method.ContainsGenericParameters) continue;
        ...
        private void AddWorldPresentationTargets()
        {
            ...
            AddTarget(typeof(Player.FirearmController), nameof(Player.FirearmController.InitiateShot), typeof(void),
            ...
            AddTarget(typeof(WeaponManagerClass), nameof(WeaponManagerClass.PlayShotEffects), typeof(void),
                new[] { typeof(bool), typeof(float) }, "Combat.WeaponManager.PlayShotEffects");
            AddTarget(typeof(EffectsCommutator), nameof(EffectsCommutator.PlayHitEffect), typeof(void),
                new[] { typeof(EftBulletClass), typeof(ShotInfoClass) }, "Combat.Effects.PlayHitEffect");
            ...
            AddTarget(typeof(AreaLight), nameof(AreaLight.SetUpCommandBuffer), typeof(void), new[] { typeof(Camera) }, "Lighting.AreaLight.BuildCommandBuffer");
            _reportBuilder.AppendLine("Lighting.AreaLight.PreCull/ManualUpdate: intentionally not invocation-timed; the previous profile measured over 150,000 calls/s and patch overhead distorted FPS. BuildCommandBuffer remains timed.");
            AddTarget(game.GetType("AmbientLight", false), "Update", typeof(void), Type.EmptyTypes, "Lighting.AmbientLight.Update");
            AddTarget(game.GetType("AmbientLight", false), "ManualOnRenderObject", typeof(void), new[] { typeof(Camera) }, "Lighting.AmbientLight.RenderObject");
            AddTarget(typeof(ObservedCullingManager), nameof(ObservedCullingManager.Update), typeof(void), Type.EmptyTypes, "Culling.ObservedPlayers.Update");
            ...
            AddTarget(game.GetType("Koenigz.PerfectCulling.PerfectCullingCamera", false), "Update", typeof(void), Type.EmptyTypes, "Culling.PerfectCamera.Update");
        }
        ...
        private void AddInstalledPluginFrameTargets()
        {
            ...
            foreach (KeyValuePair<string, BepInEx.PluginInfo> pair in Chainloader.PluginInfos)
            {
                ...
                {
                    _reportBuilder.AppendLine("Mod[" + (info.Metadata?.GUID ?? assembly.GetName().Name) + "]: type discovery skipped; " + ex.Message);
                    continue;
                }
                ...
                for (int typeIndex = 0; typeIndex < types.Length && found < maximumDiscoveredTargets; typeIndex++)
                {
                    ...
                    if (type == null || type.Assembly == typeof(Plugin).Assembly || !typeof(MonoBehaviour).IsAssignableFrom(type)) continue;
                    for (int i = 0; i < frameMethods.Length && found < maximumDiscoveredTargets; i++)
                    {
                        ...
                        if (method == null || method.ReturnType != typeof(void) || method.IsAbstract || method.ContainsGenericParameters) continue;
                        AddTarget(method, "Mod[" + guid + "]." + type.FullName + "." + frameMethods[i]);
                        found++;
                    }
                }
            }
            _reportBuilder.AppendLine("Installed-plugin MonoBehaviour frame methods discovered: " + found + " (safety cap " + maximumDiscoveredTargets + "). Inclusive nested timings must not be added together; use self time for attribution.");
        }
        ...
        private void AddDiscoveredEftSystemTargets()
        {
            ...
            {
                "Audio", "Culling", "Effect", "Ballistic", "JobScheduler", "DistantShadow", "AmbientLight", "AreaLight",
                "Decal", "Bullet", "Shell", "Weather", "GPUInstancer", "Occlusion", "Streaming", "GameWorld"
            };
            Type[] types;
            ...
            for (int typeIndex = 0; typeIndex < types.Length && found < maximumTargets; typeIndex++)
            {
                ...
                {
                    _reportBuilder.AppendLine("AutoEFT." + fullName + ": intentionally omitted from invocation timing because it runs tens of thousands of times per second.");
                    continue;
                }
                bool relevant = false;
                for (int nameIndex = 0; nameIndex < highValueNames.Length; nameIndex++)
                {
                    if (fullName.IndexOf(highValueNames[nameIndex], StringComparison.OrdinalIgnoreCase) < 0) continue;
                    relevant = true;
                    break;
                }
            ...
            _reportBuilder.AppendLine("Automatically discovered high-value EFT frame/render callbacks: " + found + " (safety cap " + maximumTargets + ").");
        }
        ...
        private void AddTarget(MethodInfo method, string displayName)
        {
            ...
            try
            {
                ...
                var finalizer = new HarmonyMethod(typeof(MethodTimingFramework).GetMethod(nameof(Finalizer), BindingFlags.Static | BindingFlags.NonPublic));
        ...
        private static void Prefix(MethodBase __originalMethod, out TimingPatchState __state)
        {
            ...
            try
            {
                if (!Lookup.TryGetValue(__originalMethod, out MethodTimingTarget target)) return;
                ThreadTimingStack stack = _threadStack ?? (_threadStack = new ThreadTimingStack());
                if (stack.Depth >= stack.Frames.Length) return;
        ...
        {
            if (!state.Active || state.Target == null || _circuitOpen) return;
            state.Active = false;
            try
            {
                ...
                if (stack != null && state.Depth >= 0 && state.Depth < stack.Frames.Length)
                {
                    ...
                    if (state.Depth > 0) stack.Frames[state.Depth - 1].ChildTicks += elapsed;
                }
        ...
        private string BuildCumulativeReport(double now, double captureSeconds, string map, string processThreadReport)
        {
            ...
            builder.AppendLine("Patched methods: " + _targets.Count + " | methods called: " + activeTargets.Count + " | measured calls: " + calls);
            builder.AppendLine("Instrumented self CPU: " + measuredSelfMs.ToString("F2", CultureInfo.InvariantCulture) + " ms (" + Percent(measuredSelfMs, durationMs) + "% of one CPU core over the capture)");
            builder.AppendLine("Instrumented inclusive CPU: main thread " + mainMs.ToString("F2", CultureInfo.InvariantCulture) + " ms | worker threads " + workerMs.ToString("F2", CultureInfo.InvariantCulture) + " ms");
            builder.AppendLine();
            ...
            builder.AppendLine("- Main/worker columns show where calls executed. Native Unity, rendering-driver and uninstrumented EFT time appears in the frame/process sections, not as a managed method.");
            builder.AppendLine();
            ...
            for (int i = 0; i < categoryList.Count; i++)
            {
                ...
                builder.Append(i + 1).Append(" | ").Append(category.Name).Append(" | ")
                    .Append(selfMs.ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(TicksToMilliseconds(category.InclusiveTicks).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(category.Calls).Append(" | ")
                    .Append(TicksToMilliseconds(category.MainTicks).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(TicksToMilliseconds(category.WorkerTicks).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(Percent(selfMs, durationMs)).AppendLine();
            }

            builder.AppendLine();
            builder.AppendLine("All instrumented methods ranked by cumulative self time");
            builder.AppendLine("rank | method | assembly | self ms | inclusive ms | calls | avg ms | p95~ ms | max ms | main ms | worker ms | one-core %");
            for (int i = 0; i < activeTargets.Count; i++)
            {
                ...
                builder.Append(i + 1).Append(" | ").Append(target.DisplayName).Append(" | ").Append(target.AssemblyName).Append(" | ")
                    .Append(selfMs.ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(inclusiveMs.ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(targetCalls).Append(" | ")
                    .Append((targetCalls > 0 ? inclusiveMs / targetCalls : 0).ToString("F5", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(target.P95Milliseconds().ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(TicksToMilliseconds(Interlocked.Read(ref target.MaximumTicks)).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(TicksToMilliseconds(Interlocked.Read(ref target.MainThreadTicks)).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(TicksToMilliseconds(Interlocked.Read(ref target.WorkerThreadTicks)).ToString("F3", CultureInfo.InvariantCulture)).Append(" | ")
                    .Append(Percent(selfMs, durationMs)).AppendLine();
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(displayName)) return "Other";
        ...
        private static string Percent(double value, double total) => (total > 0 ? value * 100.0 / total : 0).ToString("F2", CultureInfo.InvariantCulture);

        private static string Sanitize(string value)
        {
            if (string.IsNullOrWhiteSpace(value)) return "unknown-map";
        ...
        {
            internal CategoryTiming(string name) { Name = name; }
