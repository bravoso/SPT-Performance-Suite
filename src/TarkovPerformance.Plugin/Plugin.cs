...
/// <summary>
/// Coordinates configuration, raid lifecycle, diagnostics, and reversible runtime features on Unity's main thread.
/// </summary>
public sealed class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        _lifecycle.StateChanged += state =>
        {
            if (_configuration.VerboseLogging.Value)
            {
                Logger.LogInfo("Lifecycle state: " + state);
            }
        };
        Logger.LogInfo(
            $"{PluginName} {PluginVersion} loaded. The Extreme production preset is enabled by default; diagnostics and HUD overlays remain closed until requested."
        );
        _runtime.Log(Logger);
        _shadows.Initialize();
    ...
    private void Update()
    {
        ...
        try
        {
            ...
            if (_configuration.OptimizationsToggleKey.Value.IsDown())
            {
                ...
                Logger.LogWarning(
                    "A/B state changed: ALL OPTIMIZATIONS "
                        + (_configuration.OptimizationsEnabled.Value ? "ON" : "OFF")
            ...
            if (_configuration.ShadowToggleKey.Value.IsDown())
            {
                _configuration.ShadowEnabled.Value = !_configuration.ShadowEnabled.Value;
            }

            if (_configuration.SkinningToggleKey.Value.IsDown())
            {
                _configuration.SkinningEnabled.Value = !_configuration.SkinningEnabled.Value;
            }
            ...
            if (_configuration.PipReplacementToggleKey.Value.IsDown())
            {
                ...
                Logger.LogWarning(
                    "Scope mode: "
                        + (_configuration.PipReplacementEnabled.Value ? "main-camera zoom (PiP disabled)" : "full-resolution vanilla PiP")
                        + "."
                );
            }
            ...
            _timing.SetRuntimeEnabled(
                _configuration.MethodTimingEnabled.Value && (!_configuration.MethodTimingCaptureOnly.Value || _benchmark.IsCapturing)
            );
            ...
            else if (_configuration.ContinuousCaptureEnabled.Value && !_benchmark.IsCapturing)
            {
                ...
                _benchmark.Start(
                    now,
                    duration,
                    ReadMapName(_lifecycle.World),
                    FeatureState(),
                    Path.Combine(_outputRoot, "benchmarks"),
                    _configuration.ExportCsv.Value,
                    _configuration.ExportJson.Value
                );
            }
            ...
            if (_overlay.Visible || _benchmark.IsCapturing)
            {
                ...
                if (_overlay.Visible)
                {
                    _overlay.AddFrame(frameMs);
                }
            }
            bool captureCompleted =
                _benchmark.IsCapturing
                && _benchmark.Record(
                    now,
                    frameMs,
                    _metrics,
                    ...
                    default,
                    _declutter.Counters,
                    _areaLights.Counters,
                    _remoteBudget.Counters,
                    _combatPresentation.Counters,
                    _pipScopes.Counters,
                    _configuration.OptimizationsEnabled.Value,
                    _compatibility.FastLookups,
                    _fika.ServerFps
                );
            if (captureCompleted)
            {
            ...
            if (_overlay.NeedsRefresh(now))
            {
                ...
                _overlay.OptimizationSummary =
                    "Remote CPU: "
                    + RemoteBudgetStatus()
                    + "\nArea lights: "
                    + _areaLights.StatusText
                    + "\nWorld presentation: "
                    + _worldPresentation.StatusText
                    + "\nCPU threads: "
                    + _cpuThreading.StatusText
                    + "\nPiP scope: "
                    + PipScopeStatus()
                    + "\nCombat presentation: "
                    + _combatPresentation.StatusText
                    + "\nFrame pacing: "
                    + _framePacing.StatusText
                    + "\nDynamic Maps: "
                    + _dynamicMaps.StatusText
                    + "\nCompatibility: "
                    + _compatibility.StatusText
                    + "\nCombat logs: "
                    + _hotLogs.StatusText;
    ...
    private void OnGUI()
    {
        if (_configuration == null || !_configuration.Enabled.Value)
        {
            return;
        }
    ...
    {
        if (_metrics == null || _profilerMetricsActive == active)
        {
            return;
        }
        ...
        _metrics.Start();
        if (_availableProfilerMetricsWrittenForRaid)
        {
            return;
        }
        ...
        {
            _exceptions.Add("Profiler metric report", ex);
        }
    }
    ...
    private void ExportDiagnosticReport()
    {
        try
        {
            string benchmarkConfiguration =
                "durationSeconds="
                + _configuration.Validated.CaptureSeconds
                + ";exportCsv="
                + _configuration.ExportCsv.Value
                + ";exportJson="
                + _configuration.ExportJson.Value
                + ";continuous="
                + _configuration.ContinuousCaptureEnabled.Value;
            ...
            string profilePath = _timing.WriteCumulativeReport(
                Path.Combine(_outputRoot, "profiles"),
                ReadMapName(_lifecycle.World),
                profiledSeconds,
                processThreadReport
            );
            string path = DiagnosticReport.Write(
                Path.Combine(_outputRoot, "diagnostics"),
                _runtime,
                _metrics,
                _counts,
                _exceptions,
                FeatureState() + ";" + ExperimentStatus(),
                _combatPresentation.StatusText,
    ...
    {
        return "RemoteCharacterShadowLOD="
            + (_shadows != null && _shadows.IsEnabled ? "enabled" : "disabled")
            + ";OffscreenSkinning="
            + (_skinning != null && _skinning.IsEnabled ? "enabled" : "disabled")
            + ";AggressiveQuality="
            + (_aggressiveQuality != null && _aggressiveQuality.IsEnabled ? "enabled" : "disabled")
            + ";AreaLightCache="
            + (_areaLights != null && _areaLights.IsEnabled ? "enabled" : "disabled")
            + ";WorldPresentationBudget="
            + (_worldPresentation != null && _worldPresentation.IsEnabled ? "enabled" : "disabled")
            + ";CosmeticDeclutter="
            + (_declutter != null && _declutter.IsEnabled ? "enabled" : "disabled")
            + ";RemoteCpuBudget="
            + (_remoteBudget != null && _remoteBudget.IsEnabled ? "enabled" : "disabled")
            + ";AllLogicalProcessors="
            + (_cpuThreading != null && _cpuThreading.IsEnabled ? "enabled" : "disabled")
            + ";FramePacing="
            + (_framePacing != null && _framePacing.IsEnabled ? "enabled" : "disabled")
            + ";KnownModFixes="
            + (_compatibility != null && _compatibility.IsEnabled ? "enabled" : "disabled")
            + ";DynamicMapsBudget="
            + (_dynamicMaps != null && _dynamicMaps.IsEnabled ? "enabled" : "disabled")
            + ";CombatLogSuppression="
            + (_hotLogs != null && _hotLogs.IsEnabled ? "enabled" : "disabled")
            + ";CombatPresentation="
            + (_combatPresentation != null && _combatPresentation.IsEnabled ? "enabled" : "disabled")
            + ";PipScopeBudget="
            + (_pipScopes != null && _pipScopes.IsEnabled ? "enabled" : "disabled")
            + ";HeadlessAuthority="
            + (_headlessAuthority != null && _headlessAuthority.IsEnabled ? "enabled" : "disabled")
            + ";AllOptimizations="
            + (_configuration.OptimizationsEnabled.Value ? "ON" : "OFF")
            + ";ContinuousCapture="
            + _configuration.ContinuousCaptureEnabled.Value
            + ";Preset="
            + _configuration.Preset.Value;
    }

    private string ShadowStatus()
    {
        ShadowFeatureCounters counters = _shadows.Counters;
        return _shadows.StatusText
            + " | effective "
            + counters.EffectiveDistance.ToString("F0")
            + " m | AI "
            + counters.RegisteredAi
            + " beyond "
            + counters.BeyondThreshold
            + " renderers "
            + counters.TrackedRenderers
            + " disabled "
            + counters.DisabledRenderers
            + " cost "
            + counters.AverageMs.ToString("F3")
            + " ms";
    }
    ...
    private string SkinningStatus()
    {
        ...
        return _skinning.StatusText
            + " | AI "
            + counters.RegisteredAi
            + " offscreen "
            + counters.OffscreenAi
            + " candidates "
            + counters.CandidateRenderers
            + " changed "
            + counters.ModifiedRenderers
            + " cost "
            + counters.AverageMs.ToString("F3")
            + " ms";
    }
    ...
    private string DeclutterStatus()
    {
        ...
        return _declutter.StatusText
            ...
            + counters.Hidden
            + " | "
            + (counters.Complete ? "complete" : "scanning")
            + " discovery "
            + counters.DiscoveryMs.ToString("F1")
            + " ms batch "
            + counters.AverageBatchMs.ToString("F3")
            + " ms";
    }
    ...
    private string RemoteBudgetStatus()
    {
        ...
        return _remoteBudget.StatusText
            + " | remote "
            + counters.RemoteCharacters
            + " hidden "
            + counters.HiddenCharacters
            + " budgeted "
            + counters.BudgetedCharacters
            + " baked-hidden "
            + counters.BakedHiddenCharacters
            + " visible>distance "
            + counters.VisibleDistantCharacters
            + " frozen-hidden "
            + counters.FrozenHiddenCharacters
            + " animators "
            + counters.CulledAnimators
            + " skipped props "
            + counters.SkippedPropUpdates
            + " triggers "
            + counters.SkippedTriggerSearches
            + " presentation "
            + counters.SkippedPresentationUpdates
            + " | cost "
            + counters.AverageMs.ToString("F3")
            + " ms";
    }
    ...
    private string ExperimentStatus()
    {
        return "Shadows: "
            ...
            + SkinningStatus()
            + "\nAggressive quality: "
            + _aggressiveQuality.StatusText
            + "\nArea-light command cache: "
            + _areaLights.StatusText
            + "\nWorld presentation: "
            + _worldPresentation.StatusText
            + "\nDeclutter: "
    ...
    {
        if (_configuration == null || _configuration.Preset.Value == _lastPreset)
        {
            return;
        }

        _lastPreset = _configuration.Preset.Value;
        if (_lastPreset == PerformancePreset.Custom)
        {
            return;
        }
        ...
        try
        {
            PerformancePresetApplier.Apply(_configuration, _lastPreset);
        }
        finally
        {
        ...
        {
            Logger?.LogWarning("Could not save the selected performance preset: " + ex.Message);
        }
        Logger?.LogInfo(
            "Applied performance preset: "
                + _lastPreset
                + ". All optimization features are enabled; diagnostics remain independently controlled."
        );
    }
    ...
    {
        _configurationChanged = true;
        if (args?.ChangedSetting?.Definition.Section == "Aggressive")
        {
            _refreshAggressiveQuality = true;
        }
        ...
        if (
            !_applyingPreset
            && _configuration != null
            && args?.ChangedSetting != _configuration.Preset
    ...
    {
        if (!_configurationChanged || _configuration == null)
        {
            return;
        }
    ...
    private void SyncOptimizationFeatures(bool masterEnabled)
    {
        ...
        SyncFeature(
            _configuration.AggressiveModeEnabled,
            _aggressiveQuality.IsEnabled,
            _aggressiveQuality.IsAvailable,
            masterEnabled,
            _aggressiveQuality.SetEnabled
        );
        SyncFeature(
            _configuration.AreaLightCacheEnabled,
            _areaLights.IsEnabled,
            _areaLights.IsAvailable,
            masterEnabled,
            _areaLights.SetEnabled
        );
        SyncFeature(
            _configuration.WorldPresentationBudgetEnabled,
            _worldPresentation.IsEnabled,
            _worldPresentation.IsAvailable,
            masterEnabled,
            _worldPresentation.SetEnabled
        );
        SyncFeature(
            _configuration.CosmeticDeclutterEnabled,
            _declutter.IsEnabled,
            _declutter.IsAvailable,
            masterEnabled,
            _declutter.SetEnabled
        );
        SyncFeature(
            _configuration.RemoteUpdateBudgetEnabled,
            _remoteBudget.IsEnabled,
            _remoteBudget.IsAvailable,
            masterEnabled,
            _remoteBudget.SetEnabled
        );
        SyncFeature(
            _configuration.UseAllLogicalProcessors,
            _cpuThreading.IsEnabled,
            _cpuThreading.IsAvailable,
            masterEnabled,
            _cpuThreading.SetEnabled
        );
        SyncFeature(
            _configuration.FramePacingEnabled,
            _framePacing.IsEnabled,
            _framePacing.IsAvailable,
            masterEnabled,
            _framePacing.SetEnabled
        );
        SyncFeature(
            _configuration.KnownModFixesEnabled,
            _compatibility.IsEnabled,
            _compatibility.IsAvailable,
            masterEnabled,
            _compatibility.SetEnabled
        );
        SyncFeature(
            _configuration.DynamicMapsOptimizationEnabled,
            _dynamicMaps.IsEnabled,
            _dynamicMaps.IsAvailable,
            masterEnabled,
            _dynamicMaps.SetEnabled
        );
        SyncFeature(
            _configuration.HotPathLogSuppressionEnabled,
            _hotLogs.IsEnabled,
            _hotLogs.IsAvailable,
            masterEnabled,
            _hotLogs.SetEnabled
        );
        SyncFeature(
            _configuration.CombatPresentationEnabled,
            _combatPresentation.IsEnabled,
            _combatPresentation.IsAvailable,
            masterEnabled,
            _combatPresentation.SetEnabled
        );
        SyncFeature(
            _configuration.PipScopeOptimizationEnabled,
            _pipScopes.IsEnabled,
            _pipScopes.IsAvailable,
            masterEnabled,
            _pipScopes.SetEnabled
        );
        SyncFeature(
            _configuration.HeadlessAuthorityEnabled,
            _headlessAuthority.IsEnabled,
            _headlessAuthority.IsAvailable,
            masterEnabled,
            _headlessAuthority.SetEnabled
        );
    }

    private static void SyncFeature(
        ConfigEntry<bool> configuredEntry,
        bool current,
        bool available,
        bool masterEnabled,
    ...
    )
    {
        ...
        if (desired == current || (desired && !available))
        {
            return;
        }
