...
/// <summary>Reads optional Fika entity markers through cached reflection without linking Fika assemblies.</summary>
internal static class FikaEntityAdapter
{
    ...
    internal static bool IsObservedPlayer(Player player)
    {
        return player != null && player.GetType().FullName == "Fika.Core.Main.Players.ObservedPlayer";
    }
...
/// <summary>Provides Fika presence and role data to reports while remaining a soft dependency.</summary>
internal sealed class FikaDiagnosticsAdapter
{
    ...
    {
        get { return _clientType != null; }
    }

    internal int? ServerFps
    {
        get { return ReadServerFps(); }
    }

    internal void Tick(float now)
    {
        if (_client != null || now < _nextDiscovery)
        {
            return;
        }
        ...
        {
            if (
                _singletonInstantiated != null
                && _singletonInstance != null
                && _singletonInstantiated.GetValue(null, null) is bool instantiated
                && instantiated
            )
            {
    ...
    internal void GetCounts(out int observed, out int observedAi, out int visibleObservedAi)
    {
        ...
        observedAi = 0;
        visibleObservedAi = 0;
        if (_client == null || _observedPlayers == null)
        {
            return;
        }
        ...
        try
        {
            if (!(_observedPlayers.GetValue(_client, null) is IList list))
            {
                return;
            }
            ...
            {
                if (!(list[i] is Player player) || player == null)
                {
                    continue;
                }

                if (RuntimeEntityClassifier.Classify(player) != EntityKind.RemoteAI)
                {
                    continue;
                }
    ...
    {
        if (_client == null || _serverFps == null)
        {
            return null;
        }
        ...
        try
        {
            return (int)_serverFps.GetValue(_client);
        }
        catch
        {
            return null;
